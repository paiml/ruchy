    ...
    if let Ok(s) = std::str::from_utf8(data) {
        ...
        if let Ok(ast1) = parser1.parse() {
            ...
            if let Ok(rust_code) = transpiler.transpile(&ast1) {
                ...
                assert!(
                    rust_str.is_ascii()
                        || rust_str.chars().all(|c| c.is_ascii() || c.len_utf8() > 1)
                );
                ...
                {
                    assert!(
                        !rust_str.trim().is_empty(),
                        "Non-empty AST produced empty transpilation"
                    );
                }
    ...
    match &ast.kind {
        ExprKind::Function {
            ...
            assert!(
                rust_code.contains(&format!("fn {}", name))
                    || rust_code.contains(&format!("fn {name}")),
                "Function name '{}' not found in transpiled code",
                name
            );
        }
        ExprKind::Let { name, .. } => {
            ...
            assert!(
                ...
                "Let binding not found in transpiled code"
            );
        ...
        ExprKind::If { .. } => {
            assert!(
                ...
                "If expression not found in transpiled code"
            );
        }
        ExprKind::Match { .. } => {
            assert!(
                rust_code.contains("match "),
                "Match expression not found in transpiled code"
            );
        }
        ExprKind::While { .. } => {
            assert!(
                rust_code.contains("while "),
                "While loop not found in transpiled code"
            );
        }
        ExprKind::For { .. } => {
            assert!(
                rust_code.contains("for "),
                "For loop not found in transpiled code"
            );
        }
        _ => {
