...
use crate::frontend::ast::{ContractClause, Expr, ExprKind, Pattern};
use crate::frontend::lexer::Token;
use crate::frontend::parser::{bail, parse_expr_recursive, ParserState, Result};

/// Parse optional loop contract clauses (invariant/decreases) between condition and body.
/// Returns Vec<ContractClause> to attach to the loop Expr.
fn parse_loop_contracts(state: &mut ParserState) -> Result<Vec<ContractClause>> {
    let mut clauses = Vec::new();
    loop {
        match state.tokens.peek() {
            Some((Token::Invariant_, _)) => {
                state.tokens.advance();
                let condition = parse_expr_recursive(state)?;
                clauses.push(ContractClause::Invariant(Box::new(condition)));
            }
            Some((Token::Decreases, _)) => {
                state.tokens.advance();
                let expr = parse_expr_recursive(state)?;
                clauses.push(ContractClause::Decreases(Box::new(expr)));
            }
            _ => break,
        }
    }
...
fn parse_labeled_while_loop(state: &mut ParserState, label: Option<String>) -> Result<Expr> {
    ...
    if matches!(state.tokens.peek(), Some((Token::Let, _))) {
        ...
        // PMAT-001: Parse optional loop contracts (invariant/decreases)
