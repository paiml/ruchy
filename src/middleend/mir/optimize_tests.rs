use super::*;
use crate::middleend::mir::{
    AggregateKind, BasicBlock, CastKind, FieldIdx, LocalDecl, Mutability, Type,
};
...
fn test_dce_handles_switch_terminator() {
    ...
    assert!(
        dce.live_locals.contains(&disc_local),
        "Discriminant local should be live"
    );
}
...
fn test_dce_handles_call_terminator() {
    ...
    assert!(
        dce.live_locals.contains(&func_local),
        "Call func should be live"
    );
    assert!(
        dce.live_locals.contains(&arg_local),
        "Call arg should be live"
    );
    assert!(
        dce.live_locals.contains(&dest_local),
        "Call dest should be live"
    );
}
...
fn test_dce_handles_unreachable_terminator() {
    ...
    let entry_block = create_basic_block(BlockId(0), vec![], Terminator::Unreachable);

    // Block 1 is unreachable - nothing references it
    let dead_block = create_basic_block(BlockId(1), vec![], Terminator::Return(None));
    ...
    assert_eq!(func.blocks.len(), 1, "Block after Unreachable is dead code");
    assert_eq!(func.blocks[0].id, BlockId(0));
}
...
fn test_dce_return_with_none_marks_no_operand() {
    ...
    assert!(
        dce.live_locals.is_empty(),
        "Return(None) should not mark any local live"
    );
}
...
fn test_mark_rvalue_live_ref() {
    ...
    let local1 = Local(1);
    func.locals
        .push(create_local_decl(local0, Type::I32, Some("x")));
    func.locals
        .push(create_local_decl(local1, Type::I32, Some("ref_x")));
    ...
    assert_eq!(
        func.locals.len(),
        2,
        "Ref rvalue should mark referenced local live"
    );
}
...
fn test_mark_rvalue_live_ref_mutable() {
    ...
    func.locals
        .push(create_local_decl(local0, Type::I32, Some("x")));
    func.locals
        .push(create_local_decl(local1, Type::I32, Some("mut_ref_x")));
    ...
    assert_eq!(
        func.locals.len(),
        2,
        "Mutable ref should mark referenced local live"
    );
}
...
fn test_mark_rvalue_live_aggregate() {
    ...
    let local2 = Local(2);
    func.locals
        .push(create_local_decl(local0, Type::I32, Some("a")));
    func.locals
        .push(create_local_decl(local1, Type::I32, Some("b")));
    func.locals
        .push(create_local_decl(local2, Type::I32, Some("tuple")));
    ...
    assert_eq!(
        func.locals.len(),
        3,
        "Aggregate should mark all operand locals live"
    );
}
...
fn test_mark_rvalue_live_call() {
    ...
    let result_local = Local(2);
    func.locals
        .push(create_local_decl(func_local, Type::I32, Some("f")));
    func.locals
        .push(create_local_decl(arg_local, Type::I32, Some("arg")));
    func.locals
        .push(create_local_decl(result_local, Type::I32, Some("result")));
...
fn test_mark_rvalue_live_cast() {
    ...
    func.locals
        .push(create_local_decl(int_local, Type::I32, Some("n")));
    func.locals
        .push(create_local_decl(float_local, Type::F64, Some("f")));
    ...
    assert_eq!(
        func.locals.len(),
        2,
        "Cast should mark the source operand live"
    );
}
...
fn test_mark_rvalue_live_unary_op() {
    ...
    func.locals
        .push(create_local_decl(local0, Type::I32, Some("x")));
    func.locals
        .push(create_local_decl(local1, Type::I32, Some("neg_x")));
...
fn test_mark_rvalue_live_aggregate_empty() {
    ...
    func.locals.push(create_local_decl(
        result_local,
        Type::Unit,
        Some("empty_tuple"),
    ));
    ...
    assert_eq!(
        func.locals.len(),
        1,
        "Empty aggregate should still preserve result local"
    );
}
...
fn test_mark_rvalue_live_call_multiple_args() {
    ...
    let result = Local(3);
    func.locals
        .push(create_local_decl(arg0, Type::I32, Some("a")));
    func.locals
        .push(create_local_decl(arg1, Type::I32, Some("b")));
    func.locals
        .push(create_local_decl(arg2, Type::I32, Some("c")));
    func.locals
        .push(create_local_decl(result, Type::I32, Some("r")));
    ...
    assert_eq!(
        func.locals.len(),
        4,
        "Call with multiple args should mark all args live"
    );
}
