...
fn test_infer_pattern_identifier() {
    let mut ctx = make_ctx();
    let result = ctx.infer_pattern(&Pattern::Identifier("x".to_string()), &MonoType::String);
    assert!(result.is_ok());
}
...
fn test_infer_pattern_ok_with_result_type() {
    let mut ctx = make_ctx();
    let result_ty = MonoType::Result(Box::new(MonoType::Int), Box::new(MonoType::String));
    let result = ctx.infer_pattern(
...
fn test_infer_binary_op_send() {
    ...
    let result = ctx
        .infer_binary_op_type(
            BinaryOp::Send,
            &MonoType::Named("Actor".to_string()),
            &MonoType::String,
        )
...
fn test_ast_type_named_i32() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("i32 should convert");
    assert_eq!(result, MonoType::Int);
}
...
fn test_ast_type_named_i64() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("i64 should convert");
    assert_eq!(result, MonoType::Int);
}
...
fn test_ast_type_named_f32() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("f32 should convert");
    assert_eq!(result, MonoType::Float);
}
...
fn test_ast_type_named_f64() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("f64 should convert");
    assert_eq!(result, MonoType::Float);
}
...
fn test_ast_type_named_bool() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("bool should convert");
    assert_eq!(result, MonoType::Bool);
}
...
fn test_ast_type_named_string() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("String should convert");
...
fn test_ast_type_named_str() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("str should convert");
    assert_eq!(result, MonoType::String);
}
...
fn test_ast_type_named_any() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("Any should convert");
...
fn test_ast_type_named_custom() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("custom type should convert");
...
fn test_ast_type_generic_vec_with_param() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("Vec<i32> should convert");
...
fn test_ast_type_generic_list_with_param() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("List<String> should convert");
...
fn test_ast_type_generic_vec_no_params() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("Vec without params should convert");
...
fn test_ast_type_generic_option_with_param() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("Option<i32> should convert");
...
fn test_ast_type_generic_unknown_base() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("HashMap should fall back to Named");
...
fn test_ast_type_optional() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("Optional<bool> should convert");
...
fn test_ast_type_list() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("List<f64> should convert");
...
fn test_ast_type_array() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("Array should convert to List");
    assert_eq!(result, MonoType::List(Box::new(MonoType::Int)));
}
...
fn test_ast_type_function_single_param() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("fn(i32) -> bool should convert");
    assert_eq!(
        result,
...
fn test_ast_type_function_no_params() {
    ...
    let result =
        InferenceContext::ast_type_to_mono_static(&ty).expect("fn() -> i32 should convert");
...
fn test_ast_type_dataframe() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("DataFrame should convert");
    assert_eq!(
        result,
...
fn test_ast_type_series() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("Series should convert");
...
fn test_ast_type_tuple() {
    ...
    let result = InferenceContext::ast_type_to_mono_static(&ty).expect("Tuple should convert");
    assert_eq!(
        result,
...
fn test_ast_type_refined() {
    ...
    let constraint = Expr::new(ExprKind::Literal(Literal::Bool(true)), make_span());
...
fn make_df_operation_expr(source: Expr, operation: crate::frontend::ast::DataFrameOp) -> Expr {
    Expr::new(
...
fn test_infer_dataframe_filter() {
    ...
    let filter_cond = Expr::new(ExprKind::Literal(Literal::Bool(true)), make_span());
...
fn test_infer_dataframe_select() {
    ...
    let mut ctx = make_ctx();
    let source = make_df_source();
    let expr = make_df_operation_expr(source, DataFrameOp::Select(vec!["age".to_string()]));
    let result = ctx.infer_expr(&expr);
...
fn test_infer_dataframe_select_nonexistent_column() {
    ...
    let mut ctx = make_ctx();
    let source = make_df_source();
    let expr = make_df_operation_expr(source, DataFrameOp::Select(vec!["nonexistent".to_string()]));
    let result = ctx.infer_expr(&expr);
    assert!(
        result.is_ok(),
        "Select nonexistent column still succeeds with empty DataFrame"
    );
    if let Ok(MonoType::DataFrame(cols)) = result {
...
fn test_infer_dataframe_groupby() {
    ...
    let mut ctx = make_ctx();
    let source = make_df_source();
    let expr = make_df_operation_expr(source, DataFrameOp::GroupBy(vec!["age".to_string()]));
    let result = ctx.infer_expr(&expr);
...
fn test_infer_dataframe_sort() {
    ...
    let mut ctx = make_ctx();
    let source = make_df_source();
    let expr = make_df_operation_expr(source, DataFrameOp::Sort(vec!["age".to_string()]));
    let result = ctx.infer_expr(&expr);
...
fn test_infer_dataframe_operation_on_non_dataframe() {
    ...
    let source = Expr::new(ExprKind::Literal(Literal::Integer(42, None)), make_span());
    let expr = make_df_operation_expr(source, DataFrameOp::Limit(10));
    let result = ctx.infer_expr(&expr);
...
fn test_infer_dataframe_operation_on_named_dataframe() {
    ...
    let df_var = Expr::new(ExprKind::Identifier("df".to_string()), make_span());
...
fn test_infer_dataframe_macro_single_column() {
    ...
    ctx.env.bind(
        "age",
        TypeScheme::mono(MonoType::List(Box::new(MonoType::Int))),
    );
    ...
    assert!(
        result.is_ok(),
        "df! macro with single column should infer: {:?}",
        result.err()
    );
...
fn test_infer_dataframe_macro_multiple_columns() {
    ...
    ctx.env.bind(
        "name",
        TypeScheme::mono(MonoType::List(Box::new(MonoType::String))),
    );
    ctx.env.bind(
        "score",
        TypeScheme::mono(MonoType::List(Box::new(MonoType::Float))),
    );
    let args = vec![
        Expr::new(
            ExprKind::Assign {
    ...
    assert!(
        result.is_ok(),
        "df! macro with multiple columns should infer: {:?}",
        result.err()
    );
    if let Ok(MonoType::DataFrame(columns)) = &result {
...
fn test_infer_dataframe_macro_scalar_value_column() {
    ...
    assert!(
        result.is_ok(),
        "df! macro with scalar column should infer: {:?}",
        result.err()
    );
    if let Ok(MonoType::DataFrame(columns)) = &result {
...
fn test_infer_dataframe_macro_empty() {
    ...
    assert!(
        result.is_ok(),
        "Empty df! macro should produce empty DataFrame"
    );
    if let Ok(MonoType::DataFrame(columns)) = &result {
...
fn test_infer_dataframe_macro_non_assign_arg_skipped() {
    ...
    if let Ok(MonoType::DataFrame(columns)) = &result {
        assert!(
            columns.is_empty(),
            "Non-assign arg should produce empty DataFrame"
        );
    }
}
...
fn test_infer_dataframe_method_filter() {
    ...
    assert!(
        result.is_ok(),
        "filter on DataFrame should infer: {:?}",
        result.err()
    );
    assert!(matches!(result.unwrap(), MonoType::DataFrame(_)));
}
...
fn test_infer_dataframe_method_unknown_method_falls_to_generic() {
    ...
    assert!(
        result.is_ok(),
        "Unknown method should fallback to generic: {:?}",
        result.err()
    );
}
...
fn test_infer_column_selection_known_column() {
    ...
    assert!(
        result.is_ok(),
        "col('age') should infer Series(Int): {:?}",
        result.err()
    );
...
fn test_infer_column_selection_unknown_column() {
    ...
    let expr = Expr::new(
        ExprKind::MethodCall {
            ...
            method: "col".to_string(),
            args: vec![Expr::new(
                ExprKind::Literal(Literal::String("nonexistent".to_string())),
                make_span(),
            )],
        },
        make_span(),
    ...
    assert!(
        result.is_ok(),
        "col('nonexistent') should produce Series with fresh type var"
    );
...
fn test_infer_column_selection_no_args() {
    ...
    assert!(
        result.is_ok(),
        "col() with no args should produce Series with fresh var"
    );
    assert!(matches!(result.unwrap(), MonoType::Series(_)));
}
...
fn test_infer_column_selection_non_string_arg() {
    ...
    assert!(
        result.is_ok(),
        "col(0) with non-string arg should produce Series with fresh var"
    );
    assert!(matches!(result.unwrap(), MonoType::Series(_)));
}
...
fn test_infer_column_selection_on_non_dataframe() {
    let mut ctx = make_ctx();
    ctx.env
        .bind("x", TypeScheme::mono(MonoType::Named("Series".to_string())));
    let expr = Expr::new(
        ExprKind::MethodCall {
            receiver: Box::new(Expr::new(
    ...
    assert!(
        result.is_ok(),
        "col on non-DataFrame should still produce result: {:?}",
        result.err()
    );
}
...
fn test_infer_column_selection_second_column() {
    ...
    let expr = Expr::new(
        ExprKind::MethodCall {
            ...
            method: "col".to_string(),
            args: vec![Expr::new(
                ExprKind::Literal(Literal::String("y".to_string())),
                make_span(),
            )],
        },
        make_span(),
    ...
    if let Ok(MonoType::Series(inner)) = &result {
        assert_eq!(
            **inner,
            MonoType::Float,
            "Column 'y' should be Series(Float)"
        );
    }
}
