...
fn test_eval_list_expr() {
    ...
    let result = eval_list_expr(&elements, eval_expr).expect("operation should succeed in test");
...
fn test_eval_block_expr() {
    ...
    let result = eval_block_expr(&statements, eval_expr).expect("operation should succeed in test");
...
fn test_eval_tuple_expr_basic() {
    ...
    let result = eval_tuple_expr(&elements, eval_expr).expect("operation should succeed in test");
...
fn test_eval_tuple_expr_empty() {
    let eval_expr = |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::Integer(42)) };
...
fn test_eval_range_expr_inclusive() {
    ...
    let result =
        eval_range_expr(&start, &end, true, eval_expr).expect("operation should succeed in test");
    if let Value::Range {
        start: s,
        end: e,
        inclusive,
...
fn test_eval_range_expr_exclusive() {
    ...
    let result =
        eval_range_expr(&start, &end, false, eval_expr).expect("operation should succeed in test");
...
fn test_eval_loop_condition_true() {
    ...
    let mut eval_expr = |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::Bool(true)) };
    let result =
        eval_loop_condition(&condition, &mut eval_expr).expect("operation should succeed in test");
    assert!(result);
}
...
fn test_match_literal_pattern_integer() {
    ...
    assert!(
        match_literal_pattern(&lit, &Value::Integer(42)).expect("operation should succeed in test")
    );
...
fn test_eval_array_init_expr_basic() {
    ...
    let result =
        eval_array_init_expr(&element, &size, eval_expr).expect("operation should succeed in test");
    if let Value::Array(arr) = result {
        assert_eq!(arr.len(), 3);
...
fn test_eval_loop_body_continue_r126() {
    let mut eval_expr =
        |_expr: &Expr| -> Result<Value, InterpreterError> { Err(InterpreterError::Continue(None)) };
...
fn test_eval_return_expr_with_float_r126() {
    let eval_expr = |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::Float(3.14)) };
...
fn test_eval_return_expr_with_string_r126() {
    let eval_expr =
        |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::String(Arc::from("hello"))) };
