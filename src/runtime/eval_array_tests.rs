...
fn test_eval_array_nth_valid_index() {
    ...
    let result =
        eval_array_nth(&arr, &Value::Integer(1)).expect("nth should succeed for valid index");
    match result {
        Value::EnumVariant {
            enum_name,
            variant_name,
...
fn test_eval_array_nth_first_element() {
    ...
    let result = eval_array_nth(&arr, &Value::Integer(0)).expect("nth should succeed for index 0");
    match result {
        Value::EnumVariant {
            variant_name, data, ..
        } => {
...
fn test_eval_array_nth_out_of_bounds() {
    ...
    let result =
        eval_array_nth(&arr, &Value::Integer(5)).expect("nth should return None for out of bounds");
    match result {
        Value::EnumVariant {
            enum_name,
            variant_name,
...
fn test_eval_array_nth_empty_array() {
    let arr: Arc<[Value]> = Arc::from(vec![]);
    let result =
        eval_array_nth(&arr, &Value::Integer(0)).expect("nth on empty array should return None");
    match result {
        Value::EnumVariant {
            variant_name, data, ..
        } => {
...
fn test_eval_array_product_floats() {
    let arr: Arc<[Value]> = Arc::from(vec![Value::Float(2.0), Value::Float(3.0)]);
...
fn test_eval_array_product_mixed_int_float() {
    let arr: Arc<[Value]> = Arc::from(vec![Value::Integer(3), Value::Float(2.5)]);
