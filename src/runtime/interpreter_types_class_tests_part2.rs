...
fn test_instantiate_class_with_constructor_not_class_type() {
    ...
    obj.insert(
        "__type".to_string(),
        Value::from_string("NotAClass".to_string()),
    );
...
fn test_instantiate_class_with_constructor_basic() {
    ...
    let ctor_body = make_expr(ExprKind::Block(vec![make_expr(ExprKind::Literal(
...
fn test_instantiate_class_with_constructor_arg_count_mismatch() {
    ...
    let result =
        interp.instantiate_class_with_constructor("TwoArgClass", "new", &[Value::Integer(1)]);
...
fn test_instantiate_class_with_constructor_with_params() {
    ...
    let ctor = make_constructor(Some("create"), vec![make_param("val")], ctor_body);
    ...
    let result =
        interp.instantiate_class_with_constructor("ParamClass", "create", &[Value::Integer(42)]);
...
fn test_instantiate_class_with_constructor_returning_struct_literal() {
    ...
    let ctor_body = make_struct_literal(
        "StructRetClass",
        vec![(
            "x",
            make_expr(ExprKind::Literal(Literal::Integer(10, None))),
        )],
    );
    let ctor = make_constructor(Some("new"), vec![], ctor_body);
    ...
    assert!(
        ...
        "Should handle struct literal return"
    );
}
...
fn test_instantiate_class_with_constructor_unknown_constructor() {
    ...
    assert!(
        result.is_ok(),
        "Should create instance even without matching constructor: {:?}",
        result.err()
    );
}
