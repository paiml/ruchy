...
fn test_eval_array_iteration_empty_array() {
    ...
    let mut eval_expr =
        |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::Integer(99)) };

    let result = eval_array_iteration(&array, "x", &mut with_variable, &mut eval_expr)
...
fn test_eval_array_iteration_single_element() {
    ...
    let result = eval_array_iteration(&array, "item", &mut with_variable, &mut eval_expr)
...
fn test_eval_array_iteration_multiple_elements() {
    ...
    let mut with_variable =
        |_var: &str,
         val: Value,
         _eval: &mut dyn FnMut(&Expr) -> Result<Value, InterpreterError>| { Ok(val) };
    let mut eval_expr = |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::Nil) };
...
fn test_eval_array_iteration_non_array_error() {
    ...
    let result = eval_array_iteration(&not_array, "x", &mut with_variable, &mut eval_expr);
...
fn test_eval_array_iteration_nested_arrays() {
    ...
    let result = eval_array_iteration(&array, "arr", &mut with_variable, &mut eval_expr)
...
fn test_eval_range_iteration_exclusive() {
    ...
    let result = eval_range_iteration(&range, "i", &mut with_variable, &mut eval_expr)
...
fn test_eval_range_iteration_non_range_error() {
    ...
    let result = eval_range_iteration(&not_a_range, "i", &mut with_variable, &mut eval_expr);
    assert!(result.is_err());
...
fn test_eval_range_iteration_break_stops_early() {
    ...
    let _result = eval_range_iteration(&range, "i", &mut with_variable, &mut eval_expr);
}
...
fn test_eval_let_expr_basic_binding() {
    ...
    let body_expr = Expr::new(ExprKind::Identifier("x".to_string()), Span::new(0, 0));
...
fn test_eval_let_expr_string_binding() {
    ...
    let body_expr = Expr::new(ExprKind::Identifier("s".to_string()), Span::new(0, 0));
...
fn test_eval_let_expr_nil_value() {
    let value_expr = Expr::new(ExprKind::Literal(Literal::Null), Span::new(0, 0));
    let body_expr = Expr::new(ExprKind::Identifier("x".to_string()), Span::new(0, 0));
...
fn test_eval_let_expr_with_variable_uses_eval() {
    ...
    let mut eval_expr =
        |_expr: &Expr| -> Result<Value, InterpreterError> { Ok(Value::Integer(10)) };
