...
fn test_try_eval_advanced_math_part3_random() {
    let result = try_eval_advanced_math_part3("__builtin_random__", &[]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_utility_part1_range() {
    let result =
        try_eval_utility_part1("__builtin_range__", &[Value::Integer(5)]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_utility_part2_type() {
    let result =
        try_eval_utility_part2("__builtin_type__", &[Value::Integer(42)]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_utility_part2_assert() {
    let result =
        try_eval_utility_part2("__builtin_assert__", &[Value::Bool(true)]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_utility_part2_enumerate() {
    let arr = Value::Array(Arc::from(vec![Value::Integer(1)]));
    let result = try_eval_utility_part2("__builtin_enumerate__", &[arr]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_collection_function_pop() {
    let arr = Value::Array(Arc::from(vec![Value::Integer(1)]));
    let result = try_eval_collection_function("__builtin_pop__", &[arr]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_collection_function_sort() {
    ...
    let result = try_eval_collection_function("__builtin_sort__", &[arr]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_time_function_sleep() {
    let result =
        try_eval_time_function("__builtin_sleep__", &[Value::Integer(1)]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_time_function_chrono_utc_now() {
    let result = try_eval_time_function("__builtin_chrono_utc_now__", &[]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_eval_builtin_function_abs() {
    let result =
        eval_builtin_function("__builtin_abs__", &[Value::Integer(-5)]).expect("should succeed");
    assert_eq!(result, Some(Value::Integer(5)));
}

#[test]
fn test_eval_builtin_function_min() {
    let result = eval_builtin_function("__builtin_min__", &[Value::Integer(3), Value::Integer(5)])
        .expect("should succeed");
...
fn test_eval_builtin_function_max() {
    let result = eval_builtin_function("__builtin_max__", &[Value::Integer(3), Value::Integer(5)])
        .expect("should succeed");
    assert_eq!(result, Some(Value::Integer(5)));
}
...
fn test_eval_builtin_function_floor() {
    let result =
        eval_builtin_function("__builtin_floor__", &[Value::Float(3.7)]).expect("should succeed");
    assert_eq!(result, Some(Value::Integer(3)));
}
...
fn test_eval_builtin_function_ceil() {
    let result =
        eval_builtin_function("__builtin_ceil__", &[Value::Float(3.2)]).expect("should succeed");
...
fn test_eval_builtin_function_round() {
    let result =
        eval_builtin_function("__builtin_round__", &[Value::Float(3.5)]).expect("should succeed");
    assert_eq!(result, Some(Value::Integer(4)));
}
...
fn test_eval_dataframe_from_json() {
    ...
    let result =
        eval_dataframe_from_json(&[Value::from_string(json.to_string())]).expect("should succeed");
    if let Value::DataFrame { columns } = result {
        assert!(!columns.is_empty());
    } else {
        panic!("Expected DataFrame result");
...
fn test_eval_dataframe_from_json_empty() {
    let result =
        eval_dataframe_from_json(&[Value::from_string("[]".to_string())]).expect("should succeed");
    if let Value::DataFrame { columns } = result {
        assert!(columns.is_empty());
    }
}
...
fn test_eval_env_set_var_and_get() {
    ...
    // Get it back
    let result = eval_env_var(&[Value::from_string(key.to_string())]).expect("get should succeed");
...
fn test_eval_path_parent_root() {
    let result = eval_path_parent(&[Value::from_string("/".to_string())]).expect("should succeed");
    assert_eq!(result, Value::Nil);
}
...
fn test_eval_json_parse_array() {
    let result =
        eval_json_parse(&[Value::from_string("[1, 2, 3]".to_string())]).expect("should succeed");
    if let Value::Array(arr) = result {
        assert_eq!(arr.len(), 3);
    } else {
        panic!("Expected array result");
...
fn test_eval_json_parse_primitives() {
    ...
    let result = eval_json_parse(&[Value::from_string("42".to_string())]).expect("should succeed");
    assert_eq!(result, Value::Integer(42));
    ...
    let result =
        eval_json_parse(&[Value::from_string(r#""hello""#.to_string())]).expect("should succeed");
...
fn test_eval_json_validate_invalid() {
    let result =
        eval_json_validate(&[Value::from_string("not json".to_string())]).expect("should succeed");
    assert_eq!(result, Value::Bool(false));
}
...
fn test_eval_json_type() {
    let result = eval_json_type(&[Value::from_string("null".to_string())]).expect("should succeed");
    assert_eq!(result, Value::from_string("null".to_string()));

    let result = eval_json_type(&[Value::from_string("true".to_string())]).expect("should succeed");
    assert_eq!(result, Value::from_string("boolean".to_string()));

    let result = eval_json_type(&[Value::from_string("42".to_string())]).expect("should succeed");
    assert_eq!(result, Value::from_string("number".to_string()));

    let result =
        eval_json_type(&[Value::from_string(r#""hello""#.to_string())]).expect("should succeed");
    assert_eq!(result, Value::from_string("string".to_string()));

    let result = eval_json_type(&[Value::from_string("[]".to_string())]).expect("should succeed");
...
fn test_eval_fs_exists_true() {
    let result = eval_fs_exists(&[Value::from_string(".".to_string())]).expect("should succeed");
    assert_eq!(result, Value::Bool(true));
}
...
fn test_eval_fs_is_file() {
    let result =
        eval_fs_is_file(&[Value::from_string("Cargo.toml".to_string())]).expect("should succeed");
    assert_eq!(result, Value::Bool(true));

    let result = eval_fs_is_file(&[Value::from_string(".".to_string())]).expect("should succeed");
    assert_eq!(result, Value::Bool(false));
}
...
fn test_try_eval_environment_function_current_dir() {
    let result =
        try_eval_environment_function("__builtin_env_current_dir__", &[]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_try_eval_string_function_new() {
    let result = try_eval_string_function("__builtin_String_new__", &[]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_eval_builtin_function_enumerate() {
    let arr = Value::Array(Arc::from(vec![Value::Integer(1)]));
    let result = eval_builtin_function("__builtin_enumerate__", &[arr]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_eval_builtin_function_log10() {
    let result =
        eval_builtin_function("__builtin_log10__", &[Value::Float(10.0)]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_eval_builtin_function_chrono_utc_now() {
    let result = eval_builtin_function("__builtin_chrono_utc_now__", &[]).expect("should succeed");
    assert!(result.is_some());
}
...
fn test_eval_builtin_function_assert() {
    let result =
        eval_builtin_function("__builtin_assert__", &[Value::Bool(true)]).expect("should succeed");
    assert!(result.is_some());
}

#[test]
fn test_eval_builtin_function_pow() {
    let result = eval_builtin_function("__builtin_pow__", &[Value::Integer(2), Value::Integer(3)])
        .expect("should succeed");
