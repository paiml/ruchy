...
fn test_format_expr_block_with_items() {
    let f = make_formatter();
    let expr = Expr::new(ExprKind::Block(vec![int_expr(1), int_expr(2)]), span());
    let result = f.format_expr(&expr, 0);
...
fn test_format_expr_with_tabs() {
    ...
    let expr = Expr::new(ExprKind::Block(vec![int_expr(42)]), span());
    let result = f.format_expr(&expr, 0);
...
fn test_format_expr_object_literal_empty() {
    let f = make_formatter();
    let expr = Expr::new(ExprKind::ObjectLiteral { fields: vec![] }, span());
    let result = f.format_expr(&expr, 0);
    assert_eq!(result, "{}");
}
...
fn test_format_expr_if_let_no_else() {
    ...
    let expr = Expr::new(
        ExprKind::IfLet {
            ...
            then_branch: Box::new(Expr::new(ExprKind::Block(vec![ident_expr("val")]), span())),
            else_branch: None,
        },
        span(),
    );
