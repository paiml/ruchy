...
impl Transpiler {
    ...
    fn transpile_if(&self, condition: &Expr, then_branch: &Expr, else_branch: Option<&Expr>) -> Result<TokenStream> {
    ...
    fn transpile_function(&self, name: &str, params: &[Param], return_type: Option<&Type>, body: &Expr) -> Result<TokenStream> {
        let name_ident = syn::Ident::new(name, proc_macro2::Span::call_site());
        let body_tokens = self.transpile_expr(body)?;
        
        let param_tokens: Vec<TokenStream> = params.iter().map(|p| {
    ...
    fn transpile_match(&self, expr: &Expr, arms: &[MatchArm]) -> Result<TokenStream> {
        ...
        let arm_tokens: Result<Vec<_>> = arms.iter().map(|arm| {
    ...
    fn transpile_pattern(&self, pattern: &Pattern) -> Result<TokenStream> {
        Ok(match pattern {
            ...
            Pattern::List(patterns) => {
                let pattern_tokens: Result<Vec<_>> = patterns.iter()
    ...
    fn transpile_list(&self, elements: &[Expr]) -> Result<TokenStream> {
        let element_tokens: Result<Vec<_>> = elements.iter()
