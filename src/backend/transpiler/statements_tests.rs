...
use crate::frontend::ast::{BinaryOp, Expr, ExprKind, Literal, Param, Pattern, Span, Type, TypeKind, UnaryOp};
...
fn _create_transpiler_dup() -> Transpiler {
    ...
    fn test_generate_return_type_tokens_with_lifetime_ref() {
        ...
        let result = transpiler.generate_return_type_tokens_with_lifetime("test_fn", Some(&return_type), &body);
        assert!(result.is_ok());
    }
