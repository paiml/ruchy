        ...
        match &expr.kind {
            ...
            ExprKind::Function { name, params, body, .. } => {
                let param_list = params.iter()
                    .map(|p| format!("{}: {}", p.name(), "i32")) // Simplified for MVP
                    .collect::<Vec<_>>()
                    .join(", ");
                let body_code = Self::gen_expr(body)?;
            ...
            ExprKind::List(elements) => {
                let element_codes = elements.iter()
                    .map(|elem| Self::gen_expr(elem))
                    .collect::<Result<Vec<_>>>()?;
                Ok(format!("vec![{}]", element_codes.join(", ")))
            }
            
            ExprKind::Struct { name, fields, .. } => {
                let field_list = fields.iter()
                    .map(|f| format!("    {}: String,", f.name)) // Simplified for MVP
                    .collect::<Vec<_>>()
                    .join("\n");
                Ok(format!("struct {} {{\n{}\n}}", name, field_list))
            }
            ...
            ExprKind::StructLiteral { name, fields } => {
                let field_codes = fields.iter()
                    .map(|f| {
                        ...
                        Ok(format!("{}: {}", f.0, value_code))
                    })
                    .collect::<Result<Vec<_>>>()?;
                Ok(format!("{} {{ {} }}", name, field_codes.join(", ")))
            }
            ...
            ExprKind::MethodCall { receiver, method, args } => {
                ...
                let arg_codes = args.iter()
                    .map(|arg| Self::gen_expr(arg))
                    .collect::<Result<Vec<_>>>()?;
                Ok(format!("{}.{}({})", receiver_code, method, arg_codes.join(", ")))
            }
