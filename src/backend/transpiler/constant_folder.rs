...
mod tests {
    ...
    // Test 36: fold_constants - If with const false condition and no else
    #[test]
    fn test_fold_constants_if_const_false_no_else() {
        ...
        assert!(matches!(folded.kind, ExprKind::Block(ref exprs) if exprs.is_empty()));
    }

    // Test 37: fold_integer_arithmetic - division by zero (returns None)
    #[test]
    fn test_fold_integer_arithmetic_div_by_zero() {
        let result = fold_integer_arithmetic(10, BinaryOp::Divide, 0);
        assert!(result.is_none()); // Should not fold division by zero
    }

    // Test 38: fold_integer_arithmetic - checked_add overflow
    #[test]
    fn test_fold_integer_arithmetic_overflow() {
        let result = fold_integer_arithmetic(i64::MAX, BinaryOp::Add, 1);
        assert!(result.is_none()); // Should not fold on overflow
    }

    // Test 39: fold_integer_arithmetic - subtraction
    #[test]
    fn test_fold_integer_arithmetic_subtract() {
        let result = fold_integer_arithmetic(10, BinaryOp::Subtract, 3);
        assert_eq!(result, Some(Literal::Integer(7, None)));
    }

    // Test 40: fold_integer_comparison - less than
    #[test]
    fn test_fold_integer_comparison_less() {
        let result = fold_integer_comparison(5, BinaryOp::Less, 10);
        assert_eq!(result, Some(Literal::Bool(true)));
    }

    // Test 41: fold_integer_comparison - greater equal
    #[test]
    fn test_fold_integer_comparison_ge() {
        let result = fold_integer_comparison(10, BinaryOp::GreaterEqual, 10);
        assert_eq!(result, Some(Literal::Bool(true)));
    }

    // Test 42: eliminate_dead_code - If with then/else branches
    #[test]
    fn test_eliminate_dead_code_if() {
        let expr = Expr::new(
            ExprKind::If {
                condition: Box::new(Expr::new(
                    ExprKind::Literal(Literal::Bool(true)),
        ...
        let cleaned = eliminate_dead_code(expr, HashSet::new());
        // Should recursively clean branches
        assert!(matches!(cleaned.kind, ExprKind::If { .. }));
    }

    // Test 43: eliminate_dead_code - While loop
    #[test]
    fn test_eliminate_dead_code_while() {
        let expr = Expr::new(
            ExprKind::While {
                condition: Box::new(Expr::new(
                    ExprKind::Literal(Literal::Bool(true)),
        ...
        let cleaned = eliminate_dead_code(expr, HashSet::new());
        // Should recursively clean body
        assert!(matches!(cleaned.kind, ExprKind::While { .. }));
    }

    // Test 44: eliminate_dead_code - Call with arguments
    #[test]
    fn test_eliminate_dead_code_call() {
        let expr = Expr::new(
            ExprKind::Call {
                func: Box::new(Expr::new(
                    ExprKind::Identifier("foo".to_string()),
        ...
        let cleaned = eliminate_dead_code(expr, HashSet::new());
        // Should recursively clean arguments
        assert!(matches!(cleaned.kind, ExprKind::Call { .. }));
    }

    // Test 45: eliminate_dead_code - Function with body
    #[test]
    fn test_eliminate_dead_code_function() {
        let expr = Expr::new(
            ExprKind::Function {
                name: "test".to_string(),
                type_params: vec![],
        ...
        let cleaned = eliminate_dead_code(expr, HashSet::new());
        // Should recursively clean function body
        assert!(matches!(cleaned.kind, ExprKind::Function { .. }));
    }

    // Test 46: propagate_with_env - If expression
    #[test]
    fn test_propagate_with_env_if() {
        let mut env = HashMap::new();
        env.insert("x".to_string(), Literal::Integer(5, None));
        ...
        let result = propagate_with_env(expr, &mut env);
        // Variable in condition should be replaced
        if let ExprKind::If { condition, .. } = result.kind {
            assert!(matches!(condition.kind, ExprKind::Literal(Literal::Integer(5, None))));
        } else {
            panic!("Expected If expression");
        }
    }

    // Test 47: propagate_with_env - Block with multiple expressions
    #[test]
    fn test_propagate_with_env_block() {
        let mut env = HashMap::new();
        env.insert("x".to_string(), Literal::Integer(10, None));
        ...
        let result = propagate_with_env(expr, &mut env);
        // First expression should be propagated
        if let ExprKind::Block(exprs) = result.kind {
            assert_eq!(exprs.len(), 2);
            assert!(matches!(exprs[0].kind, ExprKind::Literal(Literal::Integer(10, None))));
        } else {
            panic!("Expected Block expression");
        }
    }

    // Test 48: propagate_with_env - Binary with variables
    #[test]
    fn test_propagate_with_env_binary() {
        let mut env = HashMap::new();
        env.insert("a".to_string(), Literal::Integer(2, None));
        env.insert("b".to_string(), Literal::Integer(3, None));
        ...
        let result = propagate_with_env(expr, &mut env);
        // Should propagate variables and fold: 2 * 3 → 6
        assert!(matches!(result.kind, ExprKind::Literal(Literal::Integer(6, None))));
    }

    // Test 49: fold_constants - Let with else block
    #[test]
    fn test_fold_constants_let_with_else() {
        let expr = Expr::new(
            ExprKind::Let {
                name: "x".to_string(),
                type_annotation: None,
                value: Box::new(binary(5, BinaryOp::Add, 10)),
                body: Box::new(int_lit(1)),
                is_mutable: false,
                else_block: Some(Box::new(binary(2, BinaryOp::Multiply, 3))),
            },
            Span::new(0, 0),
        );
        let folded = fold_constants(expr);
        // Should fold both value (15) and else block (6)
        if let ExprKind::Let { value, else_block, .. } = folded.kind {
            assert!(matches!(value.kind, ExprKind::Literal(Literal::Integer(15, None))));
            if let Some(else_expr) = else_block {
                assert!(matches!(else_expr.kind, ExprKind::Literal(Literal::Integer(6, None))));
            } else {
    ...
    // Test 50: fold_constants - nested If folding
    #[test]
    fn test_fold_constants_nested_if() {
        let expr = Expr::new(
            ExprKind::If {
                condition: Box::new(binary(10, BinaryOp::Greater, 5)),
                then_branch: Box::new(binary(20, BinaryOp::Add, 10)),
                else_branch: Some(Box::new(int_lit(0))),
            },
            Span::new(0, 0),
        );
        let folded = fold_constants(expr);
        // Should fold condition to true (10 > 5), then fold then-branch (30)
        assert!(matches!(folded.kind, ExprKind::Literal(Literal::Integer(30, None))));
    }
}
