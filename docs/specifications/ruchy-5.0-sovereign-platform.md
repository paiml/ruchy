...
| §14.5 falsifier scorecard (OK/WARN/FAIL per metric) | DONE | `ruchy tier` summary now ends with a single-line §14.5 scorecard: `F1:OK F2:WARN F4:WARN F11:OK`. F1: <50% FAIL / <95% WARN / ≥95% OK. F2: >5/KLoC FAIL / >0.5 WARN / ≤0.5 OK. F4: 0 OK / else WARN. F11: 0 OK / else WARN. N/A when metric cannot be evaluated (e.g., no contracts). JSON output adds `scorecard: {f1, f2, f4, f11}` nested object. 10 new handler tests + 2 new CLI tests. |
| `ruchy tier` per-file parse timeout (resilience) | DONE | Scanner now runs each file's parse in a worker thread with a 5s default timeout (configurable via `--parse-timeout-ms`). Protects against parser infinite-loops (discovered by dogfooding: examples/21_concurrency.ruchy hangs parser). Timeouts tracked in new `parse_timeouts` field of report + JSON. Dogfood scan of `examples/` (183 files) now completes in <2s despite hang. 5 new handler tests. |
| `ruchy tier --baseline <file>` regression gate | DONE | CI-friendly regression detection: first run captures baseline JSON, subsequent runs compare and exit non-zero on metric regressions (bronze/pub_bronze/errors increase, non_trivial/non_bronze pct decrease, densities increase). 0.01 tolerance on percentages/densities to ignore rounding noise. New `BaselineSnapshot` + `Regression` types + `regressions_vs()` method. 7 new handler tests + 3 new CLI tests. |
| `ruchy tier --markdown` PR-ready reports | DONE | GitHub-flavored markdown output with tier distribution table, F-metric scorecard with 🟢/🟡/🔴/⚪ badges, parser-health section (only when issues). Suitable for CI PR comments via `${ruchy tier src/ --markdown} >> $GITHUB_STEP_SUMMARY`. 3 new handler tests + 2 new CLI tests. |
| §14.5 scorecard CI gate (`--fail-on-scorecard warn\|fail`) | DONE | Eighth CI gate on `ruchy tier`. Exits status 1 with "§14.5 scorecard breach" when any metric is at-or-above the selected severity level. `FalsifierStatus::severity()` numeric ordering + `parse_level()` case-insensitive parser. Accepts warn/warning/fail/error spellings. 6 new handler tests + 3 new CLI tests. |
| `ruchy tier --config <file.toml>` centralized config | DONE | Loads gate thresholds from a TOML file with `[gates]` section. CLI flags take precedence (Option::or semantics for scalars, boolean OR for flags). Missing or malformed file errors cleanly with path context. New `TierConfig` + `TierConfigGates` serde structs. 5 new handler tests + 3 new CLI tests. |
| `ruchy contracts check/list` wired to real scanner | DONE | Previously stub commands that always reported 0 functions. Now backed by the tier scanner (reuses `provability::scan()`). `contracts check` reports real coverage % and enforces `--min-coverage` threshold (skipped when 0 functions). `contracts list` supports text/json/yaml formats and shows per-function tier + file. 3 new provability tests + 5 new sovereign tests. |
