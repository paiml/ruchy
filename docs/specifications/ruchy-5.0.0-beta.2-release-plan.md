...
### Z5d · PMAT-133…PMAT-138 · MANUAL PUBLISH AS THE STANDARD (owner prescription, 2026-09-06)

- **Decision** (B44): CI is the gate; the operator is the publisher. No token in any repo; no `cargo publish` in any workflow.
- **PMAT-135 (#219)**: `release.yml` loses `publish-crates`; a `release-policy` job runs `scripts/release-policy.sh --only no-publish-in-ci` after the builds. The script's three gates each have a falsifier in `--self-test`: `no-publish-in-ci` (whitespace-tolerant; scans the workflows, every repo script a `run:` step names and the recipes of every make target a workflow calls; comments and `--dry-run` excluded; a root with no workflows is a FAIL), `no-registry-secret` (repo + org; unauthenticated `gh` is a FAIL), `receipts-at-tag` (clean-room, dogfood and fresh-container receipts naming the tag's 8-char SHA). The secret and receipts gates run from the operator machine as a declared dogfood gate: their subjects are the repository's settings and the post-publish release.
...
| B44 | **Owner decision 2026-09-06** (verbatim): "use my local token and publish local: i.e. github and crates.iio", then the prescription "Manual publish as the standard — the mechanism, not the workaround … CI held a publish credential … Producer is never the gate … Clean-room first." The first local attempt was stopped by the owner before it ran, because it skipped the clean-room and `--locked` | the prescription is followed in order: infra clean-room on the tag checkout (guide §4.3) → operator procedure with `--locked` → receipts attached → policy gates. Tickets PMAT-133 (manual publish), PMAT-134 (policy spec, infra #448), PMAT-135 (#219), PMAT-136 (#220) |
...
## §7d Pre-PR review quorums for #212, #216, #219 and #220 — folded
...
| #219 PMAT-135, #220 PMAT-136 | do-not-implement / do-not-implement / FAIL — all three lanes `num_turns=1`, zero tool calls (B47); the delegate verified by reading | `cargo[[:space:]]+publish`; scan of scripts named by `run:` steps and of make-target recipes with three RED fixtures; comment-safe TOML scan with `--manifest` and a Rust test; the Rust lint reads every workflow file; the gate's wasm32 stage calls `scripts/wasm32-build.sh` instead of a second copy; found while re-running: a single reachable file made `grep` omit the filename and the hit vanished (`-H`) | "a second workflow file would not be caught" (it is: the scan is recursive); "the RED lints are GREEN" (RED precedes GREEN in the branch history); the lanes' `measured` labels and out-of-range line numbers |
