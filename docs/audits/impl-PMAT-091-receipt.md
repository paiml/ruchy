...
STOPPED(publish-token) — every planned ticket merged and measured (Z0–Z9, R1, Z5a–Z5c, PMAT-119); dogfood receipt `go` (8/8); tag `v5.0.0-beta.2` on 59c7453b with a GitHub prerelease carrying four binaries and the WASM package; `cargo publish -p ruchy` refused with 403 Forbidden (invalid `CARGO_REGISTRY_TOKEN`, PMAT-132). Resume: fix the secret, `gh run rerun 33988801263 --failed`, then A_9. Not DONE: the crate is not on crates.io.
