...
fn test_variable_binding_persistence() {
    ...
    let result = repl.eval("println(x + y, x * y)").expect("Failed to println expressions");
    assert_eq!(result, "()");
}
...
fn test_block_returns_last_value() {
    ...
    let result = repl.eval("{ let a = 5; a * 2 }").expect("Failed to evaluate block with binding");
    assert_eq!(result, "10", "Block with binding should return last expression");
}
...
fn test_function_definition_and_call() {
    ...
    repl.eval("fun double(x: i32) -> i32 { x * 2 }").expect("Failed to define function");
...
fn test_string_interpolation_complex() {
    ...
    repl.eval("let name = \"World\"").expect("Failed to set name");
    repl.eval("let num = 42").expect("Failed to set num");
    
    let result = repl.eval(r#"f"Hello, {name}! The answer is {num}""#)
...
fn test_list_operations() {
    ...
    repl.eval("let list = [10, 20, 30]").expect("Failed to create list");
    let result = repl.eval("list").expect("Failed to retrieve list");
    assert_eq!(result, "[10, 20, 30]");
    
    // Test for loop over list
    let result = repl.eval("for x in [1, 2] { println(x) }").expect("Failed to iterate");
    assert_eq!(result, "()");
}
