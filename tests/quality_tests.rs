...
fn test_format_integer_literal() {
    let formatter = Formatter::new();
    let expr = Expr::new(ExprKind::Literal(Literal::Integer(123)), Span::new(0, 0));
...
fn test_format_block_empty() {
    let formatter = Formatter::new();
    let expr = Expr::new(ExprKind::Block(vec![]), Span::new(0, 0));
    let result = formatter.format(&expr).unwrap();
...
fn test_formatter_fallback_for_unsupported() {
    let formatter = Formatter::new();
    // Check with a complex expression that might not be fully implemented
    let expr = Expr::new(
        ExprKind::Loop {
            body: Box::new(Expr::new(ExprKind::Block(vec![]), Span::new(0, 0))),
        },
...
// Check quality gates
use ruchy::quality::{QualityGates, QualityMetrics, QualityReport, QualityThresholds};
