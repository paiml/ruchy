...
fn test_feature(name: &str, code: &str, should_succeed: bool) -> bool {
    ...
    if success && performance_ok {
        ...
        if !performance_ok {
            println!(
                "⚠️  {}: PERFORMANCE REGRESSION ({}ms > {}ms)",
                name,
...
fn test_basic_language_features() {
    ...
    total += 1;
    if test_feature(
        "Function Definition (fn keyword)",
        r#"
fn greet(name) {
    println("Hello " + name)
}
...
fn test_one_liners() {
    ...
    let one_liners = vec![
        ...
        (
            "If Expression",
            "if 100 > 50 { \"expensive\" } else { \"cheap\" }",
        ),
        ("String Concat", "\"Hello \" + \"World\""),
        (
            "String Variables",
            "let name = \"Ruchy\"; \"Hello \" + name + \"!\"",
        ),
        ("Sqrt Method", "16.0.sqrt()"),
        (
            "Pythagorean",
            "let x = 10.0; let y = 20.0; (x * x + y * y).sqrt()",
        ),
        ("E=mc²", "let c = 299792458.0; let m = 0.1; m * c * c"),
        ("P=VI", "let v = 120.0; let i = 10.0; v * i"),
        (
            "Percentage",
            "let initial = 10000.0; let final = 15000.0; (final / initial - 1.0) * 100.0",
        ),
    ];
    ...
    assert_eq!(
        passed, total,
        "One-liner regression detected! This is a critical failure."
    );
}
