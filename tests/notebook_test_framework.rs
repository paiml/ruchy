...
use ruchy::notebook::testing::{
    NotebookTester, TestResult, TestConfig, CellTestType, 
    CellOutput, TestReport, GoldenManager
};
...
mod test_deterministic_validation {
    ...
    fn test_numeric_comparison_with_tolerance() {
        ...
        assert_eq!(result_fail, TestResult::NumericDivergence { 
            max_delta: 0.00001 
        });
    }
...
mod property_tests {
    ...
    fn prop_valid_notebooks_parse(cells: Vec<String>) -> QCResult {
        ...
        let notebook = Notebook {
            cells: cells.into_iter().enumerate().map(|(i, source)| {
                Cell {
                    id: format!("cell_{}", i),
                    source,
                    cell_type: CellType::Code,
