...
fn test_simple_execution_performance() {
    ...
    assert!(elapsed < Duration::from_secs(1), 
           "Simple execution too slow: {:?}", elapsed);
...
fn test_variable_access_performance() {
    ...
    assert!(elapsed < Duration::from_secs(2), 
           "Variable access too slow: {:?}", elapsed);
...
fn test_function_call_performance() {
    ...
    // Define function
    session.execute("func_def", "fun add(a, b) { a + b }").unwrap();
    ...
    assert!(elapsed < Duration::from_secs(1), 
           "Function calls too slow: {:?}", elapsed);
...
fn test_memory_estimation_performance() {
    ...
    // Add some data
    for i in 0..20 {
        let code = format!("let data_{} = [{}]", i, (0..i).map(|x| x.to_string()).collect::<Vec<_>>().join(", "));
...
fn test_execution_mode_switch_performance() {
    ...
    assert!(elapsed < Duration::from_millis(50), 
           "Mode switching too slow: {:?}", elapsed);
...
fn test_checkpoint_performance() {
    ...
    assert!(elapsed < Duration::from_secs(1), 
           "Checkpoint creation too slow: {:?}", elapsed);
...
fn test_scalability_characteristics() {
    ...
    for &size in &test_sizes {
        ...
        for i in 0..size {
            ...
            session.execute(&format!("scale_setup_{}_{}", size, i), &code).unwrap();
        }
        ...
        session.execute("scale_access", &format!("scale_var_{}", size / 2)).unwrap();
    ...
    for (i, &time) in times.iter().enumerate() {
        println!("Scale test {}: {} variables, access time: {:?}", i, test_sizes[i], time);
        
        // Each access should be fast regardless of total variables
        assert!(time < Duration::from_millis(10), 
               "Variable access scales poorly: {:?} for {} variables", time, test_sizes[i]);
    }
...
fn test_baseline_performance_metrics() {
    ...
    let (_, simple_exec_time) = measure_execution(|| {
        session.execute("baseline", "42").unwrap()
    });
    println!("📊 Simple execution: {:?}", simple_exec_time);
    
    let (_, memory_time) = measure_execution(|| {
        session.estimate_interpreter_memory()
    });
    println!("📊 Memory estimation: {:?}", memory_time);
    
    // Define performance baseline thresholds
    assert!(creation_time < Duration::from_millis(10), "Session creation baseline");
    assert!(simple_exec_time < Duration::from_millis(10), "Simple execution baseline");
    assert!(memory_time < Duration::from_micros(100), "Memory estimation baseline");
