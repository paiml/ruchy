...
fn test_in_repl(code: &str) -> Result<String, String> {
    let output = Command::new("bash")
        .arg("-c")
        .arg(&format!("echo '{}' | ./target/release/ruchy repl 2>&1", code))
        .output()
...
fn validate_method(name: &str, code: &str, expected: &str) -> Result<(), String> {
    ...
    if !repl_result.contains(expected) {
        return Err(format!("{}: REPL returned '{}', expected '{}'", name, repl_result, expected));
    }
    ...
    match test_in_file(code) {
        Ok(file_result) => {
            if file_result != expected {
                return Err(format!("{}: File execution returned '{}', expected '{}'", 
                    name, file_result, expected));
            }
            Ok(())
        }
        Err(e) => {
            // If file execution fails, it means transpiler doesn't support it
            Err(format!("{}: Works in REPL but NOT in transpiler: {}", name, e))
        }
    }
}
...
fn validate_string_methods() {
    ...
    if !failures.is_empty() {
        panic!("Standard library validation failures:\n{}", failures.join("\n"));
    }
}
...
fn validate_array_methods() {
    ...
    if !failures.is_empty() {
        panic!("Array method validation failures:\n{}", failures.join("\n"));
    }
}
...
fn validate_number_methods() {
    ...
    if !failures.is_empty() {
        panic!("Number method validation failures:\n{}", failures.join("\n"));
    }
}
...
fn validate_object_methods() {
    ...
    if !failures.is_empty() {
        panic!("Object method validation failures:\n{}", failures.join("\n"));
    }
}