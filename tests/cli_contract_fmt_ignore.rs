...
let y = 3 + 4",
    ...
    assert!(
        ...
        "Ignored line should preserve exact formatting"
    );

    // Second line should be formatted normally
    assert!(
        formatted.contains("let y = 3 + 4"),
        "Non-ignored line should be formatted"
    );
}
...
let x    =    1  +  2",
    ...
    assert!(
        formatted.contains("let x    =    1  +  2"),
        "ruchy-fmt-ignore-next should work like ruchy-fmt-ignore"
    );
}
...
let d = 4",
    ...
    assert!(
        ...
        "First ignored line should preserve formatting"
    );
    assert!(
        formatted.contains("let c    =    3"),
        "Second ignored line should preserve formatting"
    );
...
fn bar(a, b) { a + b }",
    ...
    assert!(
        formatted.contains("fn foo ( x,y ) { x+y }"),
        "Ignored function should preserve exact formatting"
    );

    // Non-ignored function is formatted
    assert!(
        formatted.contains("fn bar(a, b)"),
        "Non-ignored function should be formatted"
    );
}
...
let x  =  1  +  2   // trailing comment
    ...
    assert!(
        formatted.contains("let x  =  1  +  2   // trailing comment"),
        "Should preserve exact formatting including comments"
    );
}
...
let x    =    1  +  2",
    ...
    // Format both files
    ruchy_cmd().arg("fmt").arg(&ignored_file).assert().success();

    ruchy_cmd().arg("fmt").arg(&normal_file).assert().success();
    ...
    assert!(
        ignored_content.contains("let x    =    1  +  2"),
        "Ignored file should preserve formatting"
    );

    // Normal file is formatted
    assert!(
        normal_content.contains("let y = 3 + 4"),
        ...
        r"// ruchy-fmt-ignore
{
    ...
    assert!(
        formatted.contains("let a=1"),
        "Nested expressions in ignored block should preserve formatting"
    );
    assert!(
        formatted.contains("let b=2"),
        "Nested expressions in ignored block should preserve formatting"
...
let x    =    1  +  2",
    ...
    assert!(
        formatted.contains("let x = 1 + 2"),
        "Uppercase directive should not be recognized"
    );
}
...
let x    =    1  +  2",
    ...
    assert!(
        formatted.contains("let x    =    1  +  2"),
        "Directive with whitespace should be recognized"
    );
}
