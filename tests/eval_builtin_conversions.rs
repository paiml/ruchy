...
fn test_int_from_string_positive() {
    ruchy_cmd().arg("-e").arg("println(int(\"123\"))")
        .assert().success().stdout(predicate::str::contains("123"));
}

#[test]
fn test_int_from_string_negative() {
    ruchy_cmd().arg("-e").arg("println(int(\"-456\"))")
        .assert().success().stdout(predicate::str::contains("-456"));
}

#[test]
fn test_int_from_float_truncate() {
    ruchy_cmd().arg("-e").arg("println(int(3.7))")
...
fn test_float_from_string() {
    ruchy_cmd().arg("-e").arg("println(float(\"3.14\"))")
        .assert().success().stdout(predicate::str::contains("3.14"));
}

#[test]
fn test_float_from_string_integer() {
    ruchy_cmd().arg("-e").arg("println(float(\"42\"))")
        .assert().success().stdout(predicate::str::contains("42"));
}
...
fn test_float_from_boolean_true() {
    ruchy_cmd().arg("-e").arg("println(float(true))")
        .assert().success().stdout(predicate::str::contains("1"));
}

#[test]
fn test_float_from_boolean_false() {
    ruchy_cmd().arg("-e").arg("println(float(false))")
        .assert().success().stdout(predicate::str::contains("0"));
}
...
fn test_bool_from_integer_nonzero() {
    ruchy_cmd().arg("-e").arg("println(bool(42))")
        .assert().success().stdout(predicate::str::contains("true"));
}

#[test]
fn test_bool_from_integer_zero() {
    ruchy_cmd().arg("-e").arg("println(bool(0))")
        .assert().success().stdout(predicate::str::contains("false"));
}

#[test]
fn test_bool_from_float_nonzero() {
    ruchy_cmd().arg("-e").arg("println(bool(3.14))")
        .assert().success().stdout(predicate::str::contains("true"));
}

#[test]
fn test_bool_from_float_zero() {
    ruchy_cmd().arg("-e").arg("println(bool(0.0))")
        .assert().success().stdout(predicate::str::contains("false"));
}

#[test]
fn test_bool_from_string_nonempty() {
    ruchy_cmd().arg("-e").arg("println(bool(\"hello\"))")
        .assert().success().stdout(predicate::str::contains("true"));
}

#[test]
fn test_bool_from_string_empty() {
    ruchy_cmd().arg("-e").arg("println(bool(\"\"))")
        .assert().success().stdout(predicate::str::contains("false"));
}

#[test]
fn test_bool_from_boolean() {
    ruchy_cmd().arg("-e").arg("println(bool(true), bool(false))")
        .assert().success()
...
fn test_sleep_milliseconds() {
    ruchy_cmd().arg("-e").arg("sleep(10); println(\"awake\")")
        .assert().success().stdout(predicate::str::contains("awake"));
}

#[test]
fn test_sleep_zero() {
    ruchy_cmd().arg("-e").arg("sleep(0); println(\"immediate\")")
        .assert().success().stdout(predicate::str::contains("immediate"));
}

#[test]
fn test_timestamp_returns_number() {
    // timestamp() returns a number (milliseconds since epoch)
    ruchy_cmd().arg("-e").arg("let t = timestamp(); assert(t > 0)")
        .assert().success();
}
...
fn test_timestamp_increases() {
    ...
    ruchy_cmd().arg("-e").arg("let t1 = timestamp(); sleep(1); let t2 = timestamp(); assert(t2 >= t1)")
        .assert().success();
}
...
fn edge_case_int_max_string_length() {
    ...
    ruchy_cmd().arg("-e").arg("println(int(\"999999999\"))")
        .assert().success().stdout(predicate::str::contains("999999999"));
}

#[test]
fn edge_case_float_scientific_notation() {
    // Scientific notation (if supported)
    ruchy_cmd().arg("-e").arg("println(float(\"1.5e2\"))")
        .assert().success();
}
...
fn error_int_invalid_string() {
    ruchy_cmd().arg("-e").arg("let x = int(\"not_a_number\")")
        .assert().failure();
}

#[test]
fn error_float_invalid_string() {
    ruchy_cmd().arg("-e").arg("let x = float(\"invalid\")")
        .assert().failure();
}
