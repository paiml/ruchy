...
fn test_reference_operator_parsing() {
    ...
    assert!(result.is_ok(), "Reference operator should parse successfully: {:?}", result);
    
    // Test reference with variable
    repl.eval("let x = 10").unwrap();
    let result = repl.eval("&x");
    if let Err(e) = &result {
        println!("Error parsing &x: {}", e);
    }
    assert!(result.is_ok(), "Reference to variable should work: {:?}", result);
}
...
fn test_reference_operator_priority() {
    ...
    assert!(result.is_ok(), "Reference should work with other unary operators");
...
fn test_reference_operator_transpilation() {
    ...
    use ruchy::frontend::ast::{Expr, ExprKind, Literal, UnaryOp, Span};
