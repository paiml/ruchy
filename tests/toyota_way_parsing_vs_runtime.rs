...
fn toyota_way_parse_vs_runtime_separation() {
    let test_cases = vec![
        ...
        ("1 + 2", true, true, "Simple arithmetic - both parse and run"),
        ("Result::Ok(42)", true, true, "Known constructor - both parse and run"),
        ("std::fs::read_file(\"test\")", true, false, "Unknown method - parse OK, runtime fail"),
        ("invalid::syntax::}", false, false, "Invalid syntax - parse fails"),
        ("println(\"hello\")", true, true, "Known function - both parse and run"),
    ];
    ...
    for (input, should_parse, should_run, description) in test_cases {
        ...
        if parse_succeeded {  // Only test runtime if parsing succeeded
            ...
            let runtime_error = stderr.contains("Error:") && !stderr.contains("Compiling") && !stderr.contains("Finished");
            ...
            println!("  ✅ Parse: {}, Runtime: {} (expected: {})", 
...
fn toyota_way_parsing_performance_consistency() {
    ...
    let input = "std::collections::HashMap<String, Vec<Result<Option<std::fs::File>, std::io::Error>>>";
    let iterations = 1000;
    ...
    for _ in 0..iterations {
        let mut parser = Parser::new(input);
        let result = parser.parse();
        assert!(result.is_ok(), "Performance test input should parse successfully");
    }
