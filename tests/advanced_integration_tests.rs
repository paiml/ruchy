...
fn test_complex_data_structure_pipeline() {
    ...
    // Calculate sum
    let result2 = repl.eval("let mut sum = 0; for row in matrix { for val in row { sum = sum + val } }; sum");
...
fn test_large_function_compilation_performance() {
    ...
    assert!(parse_time.as_millis() < 1000, "Parsing took too long: {}ms", parse_time.as_millis());
    ...
    assert!(transpile_time.as_millis() < 2000, "Transpiling took too long: {}ms", transpile_time.as_millis());
}
...
fn test_recursive_type_inference_performance() {
    ...
    assert!(elapsed.as_millis() < 500, "Type inference took too long: {}ms", elapsed.as_millis());
}
...
fn test_memory_leak_prevention() {
    ...
    assert!(memory_used < 100_000_000, "Memory usage too high: {} bytes", memory_used);
}
