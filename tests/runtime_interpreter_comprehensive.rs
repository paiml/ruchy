...
fn test_local_scope_function() {
    ruchy_cmd()
        .arg("-e")
        .arg("fn test() { let x = 20; x }; println(test())")
        .assert()
        .success()
...
fn test_shadowing() {
    ruchy_cmd()
        .arg("-e")
        .arg("let x = 10; let x = 20; println(x)")
        .assert()
        .success()
        .stdout(predicate::str::contains("20"));
}
...
fn test_closure_captures_environment() {
    ruchy_cmd()
        .arg("-e")
        .arg("let x = 42; let f = || x; println(f())")
        .assert()
        .success()
...
fn test_eval_nested_if() {
    ruchy_cmd()
        .arg("-e")
        .arg("if true { if true { println(\"nested\") } }")
        .assert()
        .success()
...
fn test_eval_match_literal() {
    ruchy_cmd()
        .arg("-e")
        .arg("let x = match 2 { 1 => 10, 2 => 20, _ => 30 }; println(x)")
        .assert()
        .success()
        .stdout(predicate::str::contains("20"));
}
...
fn test_eval_match_default() {
    ruchy_cmd()
        .arg("-e")
        .arg("let x = match 5 { 1 => 10, 2 => 20, _ => 30 }; println(x)")
        .assert()
        .success()
...
fn test_eval_for_loop() {
    ruchy_cmd()
        .arg("-e")
        .arg("let mut sum = 0; for i in range(5) { sum = sum + i }; println(sum)")
        .assert()
        .success()
        .stdout(predicate::str::contains("10"));
}
...
fn test_eval_while_loop() {
    ruchy_cmd()
        .arg("-e")
        .arg("let mut i = 0; while i < 5 { i = i + 1 }; println(i)")
        .assert()
        .success()
...
fn test_eval_loop_with_break() {
    ruchy_cmd()
        .arg("-e")
        .arg("let mut i = 0; loop { i = i + 1; if i > 3 { break } }; println(i)")
        .assert()
        .success()
...
fn test_eval_function_early_return() {
    ruchy_cmd()
        .arg("-e")
        .arg("fn test() { return 42; 99 }; println(test())")
        .assert()
        .success()
        .stdout(predicate::str::contains("42"));
}
...
fn test_eval_recursive_function() {
    ruchy_cmd()
        .arg("-e")
        .arg("fn fact(n) { if n <= 1 { 1 } else { n * fact(n-1) } }; println(fact(5))")
        .assert()
        .success()
...
fn test_eval_closure() {
    ruchy_cmd()
        .arg("-e")
        .arg("let add5 = |x| x + 5; println(add5(10))")
        .assert()
        .success()
...
fn test_eval_higher_order_function() {
    ruchy_cmd()
        .arg("-e")
        .arg("fn apply(f, x) { f(x) }; let double = |n| n * 2; println(apply(double, 21))")
        .assert()
        .success()
        .stdout(predicate::str::contains("42"));
}
...
fn test_eval_nested_array() {
    ruchy_cmd()
        .arg("-e")
        .arg("let arr = [[1, 2], [3, 4]]; println(arr[0][1])")
        .assert()
        .success()
...
fn test_eval_f_string_single_var() {
    ruchy_cmd()
        .arg("-e")
        .arg("let x = 42; println(f\"value: {x}\")")
        .assert()
        .success()
...
fn test_eval_f_string_with_expression() {
    ruchy_cmd()
        .arg("-e")
        .arg("let n = 5; println(f\"{n} squared is {n*n}\")")
        .assert()
        .success()
        .stdout(predicate::str::contains("5"))
...
fn test_eval_index_out_of_bounds() {
    ruchy_cmd()
        .arg("-e")
        .arg("let arr = [1, 2, 3]; println(arr[10])")
        .assert()
        .failure();
}
...
fn test_eval_string_multiplication() {
    ...
    ruchy_cmd()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("hellohellohellohellohello"));
}
...
fn test_eval_string_to_int() {
    ruchy_cmd()
        .arg("-e")
        .arg("let s = \"123\"; let n = int(s); println(n)")
        .assert()
        .success()
...
fn property_variable_assignment() {
    ...
    for val in [0, 42, -100, 999] {
        ruchy_cmd()
            .arg("-e")
            .arg(format!("let x = {val}; assert_eq(x, {val})"))
            .assert()
            .success();
    }
}
...
fn property_function_return_values() {
    ...
    for val in [1, 10, 100] {
        ruchy_cmd()
            .arg("-e")
            .arg(format!("fn f() {{ {val} }}; assert_eq(f(), {val})"))
            .assert()
            .success();
    }
}
...
fn property_closure_captures() {
    ...
    for val in [5, 10, 20] {
        ruchy_cmd()
            .arg("-e")
            .arg(format!(
                "let x = {val}; let f = || x; assert_eq(f(), {val})"
            ))
            .assert()
            .success();
    }
...
fn edge_case_zero_iterations() {
    ruchy_cmd()
        .arg("-e")
        .arg("for i in range(0) { println(\"never\") }; println(\"done\")")
        .assert()
        .success()
...
fn edge_case_deeply_nested_calls() {
    ruchy_cmd()
        .arg("-e")
        .arg("fn a() { b() }; fn b() { c() }; fn c() { 42 }; println(a())")
        .assert()
        .success()
        .stdout(predicate::str::contains("42"));
}
...
fn edge_case_recursion_depth_limit() {
    ...
    ruchy_cmd()
        .arg("-e")
        .arg("fn count(n) { if n == 0 { 0 } else { 1 + count(n-1) } }; println(count(50))")
        .assert()
        .success()
...
fn edge_case_recursion_limit_exceeded() {
    ...
    ruchy_cmd()
        .arg("-e")
        .arg("fn count(n) { if n == 0 { 0 } else { 1 + count(n-1) } }; println(count(100))")
        .assert()
        .failure()
