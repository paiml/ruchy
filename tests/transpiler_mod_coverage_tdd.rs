...
use ruchy::frontend::ast::{MatchArm, Param, Pattern, Span, Type, TypeKind};
use ruchy::Transpiler;
use ruchy::{BinaryOp, Expr, ExprKind, Literal};
...
fn test_type_to_string_named() {
    ...
    transpiler.collect_function_signatures(&[func]);
    assert_eq!(
        transpiler.function_signatures["test"].param_types[0],
        "String"
    );
}
...
fn test_type_to_string_reference() {
    ...
    transpiler.collect_function_signatures(&[func]);
    assert_eq!(
        transpiler.function_signatures["ref_test"].param_types[0],
        "&Int"
    );
}
