...
memory_test!(test_repeated_allocation_deallocation, {
    // Test that repeated allocation and deallocation doesn't leak memory
...
memory_test!(test_parser_memory_cleanup, {
    // Test that parser doesn't leak memory across multiple uses
    for i in 0..50 {
...
memory_test!(test_transpiler_memory_stability, {
    // Test that transpiler maintains stable memory usage
...
mod wasm_specific_memory_tests {
    ...
    fn test_wasm_string_allocation() {
        // Test string allocation patterns common in WASM FFI
        for i in 0..100 {
    ...
    fn test_array_buffer_management() {
        // Test array buffer allocation and cleanup
    ...
    fn test_function_call_memory_overhead() {
        // Test that repeated function calls don't accumulate memory
...
mod native_memory_tests {
    ...
    fn test_large_input_memory_handling() {
        // Test that large inputs are handled without excessive memory usage
    ...
    fn test_recursive_structure_memory() {
        // Test deeply nested structures don't cause memory issues
