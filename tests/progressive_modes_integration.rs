...
fn test_contextual_features_per_mode() {
    ...
    // Check mode should format results with checkmarks
...
fn test_all_mode_types() {
    ...
    // Check all available modes can be activated
    let modes = [
        ("normal", "ruchy> "),
        ("test", "test> "),
        ("debug", "debug> "),
    ...
    // Check modes help command works
...
fn test_progressive_activation_edge_cases() {
    ...
    // Check that mode persists until changed
    let result = repl.eval("100").unwrap();
    assert_eq!(repl.get_mode(), "debug");
    assert!(result.contains("┌─ Trace"));
}
