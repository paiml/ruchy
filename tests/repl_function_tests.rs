...
fn test_function_with_local_vars() {
    ...
    let result = repl.eval("fun multiply_and_add(x: i32, y: i32) -> i32 { let prod = x * y; prod + 10 }");
    assert!(result.is_ok());
...
fn test_function_wrong_arg_count() {
    ...
    assert!(result.unwrap_err().to_string().contains("expects 2 arguments, got 1"));
}
...
fn test_multiple_functions() {
    ...
    assert!(repl.eval("fun add(a: i32, b: i32) -> i32 { a + b }").is_ok());
    assert!(repl.eval("fun sub(a: i32, b: i32) -> i32 { a - b }").is_ok());
    assert!(repl.eval("fun mul(a: i32, b: i32) -> i32 { a * b }").is_ok());
...
fn test_function_using_global_variable() {
    ...
    assert!(repl.eval("fun use_global(x: i32) -> i32 { x + global }").is_ok());
