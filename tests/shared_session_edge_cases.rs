...
fn test_recursive_function() {
    ...
    let result = session.execute("factorial", "fun fact(n) { if n <= 1 { 1 } else { n * fact(n - 1) } }");
    assert!(result.is_ok());
...
fn test_mutual_recursion() {
    ...
    session.execute("even", "fun is_even(n) { if n == 0 { true } else { is_odd(n - 1) } }").ok();
    session.execute("odd", "fun is_odd(n) { if n == 0 { false } else { is_even(n - 1) } }").ok();
...
fn test_memory_estimation_changes() {
    ...
    // Add some data
    session.execute("data1", "let arr = [1, 2, 3, 4, 5]").unwrap();
    ...
    // Add more data
    session.execute("data2", r#"let str = "a very long string that takes up memory""#).unwrap();
