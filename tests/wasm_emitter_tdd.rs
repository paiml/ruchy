...
fn test_emit_integer_literal() {
    ...
    assert!(wasm_bytes.windows(2).any(|w| w[0] == 0x41 && w[1] == 42),
...
fn test_emit_addition() {
    ...
    assert!(wasm_bytes.windows(2).any(|w| w[0] == 0x41 && w[1] == 2),
            "Should contain i32.const 2");
    assert!(wasm_bytes.windows(2).any(|w| w[0] == 0x41 && w[1] == 3),
            "Should contain i32.const 3");
    assert!(wasm_bytes.contains(&0x6a),
...
fn test_emit_function() {
    ...
    assert!(wasm_bytes.windows(2).any(|w| w[0] == 3),
...
fn test_emit_executable_main() {
    ...
    // Export section = 0x07
    assert!(wasm_bytes.windows(2).any(|w| w[0] == 0x07),
...
fn test_emit_local_variables() {
    ...
    assert!(wasm_bytes.contains(&0x21),
            "Should contain local.set instruction");
    assert!(wasm_bytes.contains(&0x20),
...
fn test_emit_arithmetic_ops() {
    ...
    for (source, opcode, op_name) in operations {
        ...
        let ast = parser.parse()
            .expect(&format!("Should parse {}", source));
        ...
        assert!(wasm_bytes.contains(&opcode),
                "Should contain {} instruction (0x{:02x})", op_name, opcode);
    }
}
...
fn test_emit_comparison_ops() {
    ...
    for (source, opcode, op_name) in comparisons {
        ...
        assert!(wasm_bytes.contains(&opcode),
                "Should contain {} instruction", op_name);
    }
}
...
fn test_emit_if_else() {
    ...
    assert!(wasm_bytes.contains(&0x05), "Should contain else instruction");
...
fn test_emit_loop() {
    ...
    assert!(wasm_bytes.contains(&0x03), "Should contain loop instruction");
    assert!(wasm_bytes.contains(&0x0d), "Should contain br_if instruction");
}
...
fn test_emit_function_call() {
    ...
    // call = 0x10
    assert!(wasm_bytes.contains(&0x10), "Should contain call instruction");
}
...
fn test_emit_return() {
    ...
    // return = 0x0f
    assert!(wasm_bytes.contains(&0x0f), "Should contain return instruction");
}
...
fn test_emit_memory_section() {
    ...
    // Memory section = 0x05
    assert!(wasm_bytes.windows(2).any(|w| w[0] == 0x05),
...
fn test_emit_complete_program() {
    ...
    let module = wasmtime::Module::new(&engine, &wasm_bytes)
        .expect("Should create wasmtime module");
...
mod property_tests {
    ...
    fn prop_arithmetic_expressions_valid(a: i8, b: i8, op: u8) -> bool {
        ...
        let b_safe = if b == 0 && (op_str == "/" || op_str == "%") { 1 } else { b };
