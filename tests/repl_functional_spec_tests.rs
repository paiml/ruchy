...
fn test_for_loops() {
    ...
    assert!(repl
        .eval("for i in [1, 2, 3, 4, 5] { sum = sum + i }")
        .is_ok());
...
fn test_match_expressions() {
    ...
    assert_eq!(
        repl.eval("match 2 { 1 => \"one\", 2 => \"two\", _ => \"other\" }")
            .unwrap(),
        "\"two\""
    );
    ...
    assert_eq!(
        repl.eval("match x { n if n > 5 => \"big\", _ => \"small\" }")
            .unwrap(),
        "\"big\""
    );
}
...
fn test_objects() {
    ...
    // Object literal
    assert!(repl
        .eval("let person = { name: \"Alice\", age: 30 }")
        .is_ok());
...
fn test_string_interpolation() {
    ...
    assert_eq!(
        repl.eval("f\"The answer is {x}\"").unwrap(),
        "\"The answer is 42\""
    );
}
...
fn test_factorial_calculation() {
    ...
    // Recursive factorial
    assert!(repl
        .eval("fn factorial(n) { if n <= 1 { 1 } else { n * factorial(n - 1) } }")
        .is_ok());
...
fn test_fibonacci() {
    ...
    assert!(repl
        .eval("fn fib(n) { if n <= 1 { n } else { fib(n - 1) + fib(n - 2) } }")
        .is_ok());
...
fn test_higher_order_functions() {
    ...
    // Filter function
    assert!(repl
        .eval("let evens = nums.filter(x => x % 2 == 0)")
        .is_ok());
    assert_eq!(repl.eval("evens").unwrap(), "[2, 4]");

    // Reduce function
    assert!(repl
        .eval("let sum = nums.reduce((a, b) => a + b, 0)")
        .is_ok());
    assert_eq!(repl.eval("sum").unwrap(), "15");
}
