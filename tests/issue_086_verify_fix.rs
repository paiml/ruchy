...
fn test_issue_086_fix_verification_100_iterations() {
    ...
    for i in 1..=100 {
        ...
        *hash_frequency.entry(result2.state_hash.clone()).or_insert(0) += 1;
        ...
        if result1.state_hash == result2.state_hash {
            ...
            mismatch_count += 1;
            if mismatch_count <= 5 {
                println!("Run {}: ✗ MISMATCH (hash1: {}..., hash2: {}...)",
                    i,
    ...
    if hash_frequency.len() > 1 {
        println!("\n⚠️  NON-DETERMINISM STILL PRESENT: {} different hashes", hash_frequency.len());
    ...
    assert_eq!(match_count, 100, "All 100 runs should produce identical hashes");
}
