...
fn test_ubuntu_config_basic_if_expression() {
    // Test pattern from ubuntu-config-scripts: if expression without semicolon
...
fn test_ubuntu_config_comparison_operators() {
    // Test various comparison patterns used in ubuntu-config-scripts
    let examples = vec![
        "2 + 3 == 5",
        "result == 5",
        "10 > 5", 
...
fn test_ubuntu_config_string_literals() {
    // Test string patterns from ubuntu-config-scripts
    let examples = vec![
        r#""✅ Test passed""#,
        r#""❌ Test failed""#,
        r#""Hello, World!""#,
...
fn test_ubuntu_config_function_definition() {
    // Test function definition pattern from ubuntu-config-scripts
...
fn test_ubuntu_config_real_world_patterns() {
    // Test actual patterns from ubuntu-config-scripts files
    let patterns = vec![
        // Basic computation
        ...
        ("let config = { name: \"test\", version: 1 }", "object literal"),
    ];
...
fn test_ubuntu_config_block_expressions() {
    // Test that block expressions work (needed for function bodies)
