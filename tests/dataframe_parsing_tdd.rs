...
mod dataframe_parsing_tdd {
    ...
    fn test_df_empty_brackets_should_parse() {
        ...
        let mut repl =
            ruchy::runtime::Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
        let deadline = Instant::now() + std::time::Duration::from_secs(1);

        // Check that df![] parses and executes without error
    ...
    fn test_df_empty_in_assignment() {
        ...
        // Check that df![] can be assigned to a variable
