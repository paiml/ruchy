...
use ruchy::runtime::repl_v3::{ReplConfig, ReplV3};
...
fn test_state_transitions() {
    use ruchy::runtime::repl_v3::state::{Environment, State};
...
fn test_error_recovery() {
    ...
    assert!(matches!(
        action,
        ruchy::runtime::repl_v3::recovery::RestartAction::UseValue(_)
    ));
}
...
fn test_parser_integration() {
    ...
    let result = repl
        .evaluator
        .eval("10 - 3")
        .expect("Should evaluate subtraction");
    assert_eq!(result, "7");

    let result = repl
        .evaluator
        .eval("4 * 5")
        .expect("Should evaluate multiplication");
    assert_eq!(result, "20");

    let result = repl
        .evaluator
        .eval("15 / 3")
        .expect("Should evaluate division");
    assert_eq!(result, "5");
