...
fn test_fstring_interpolation_variable_usage() {
    ...
    // Check that variables used in f-string interpolations are recognized as used
    let code = r#"
let name = "World"
...
fn test_multiple_fstring_interpolations() {
    ...
    // Check multiple variables in f-strings
    let code = r#"
let first = "John"
let last = "Doe"
let age = 30
...
fn test_nested_fstring_expressions() {
    ...
    // Check expressions within f-string interpolations
    let code = r#"
let x = 5
let y = 10
...
fn test_function_parameter_usage_in_body() {
    ...
    // Check that function parameters used in the body are recognized
    let code = r"
fn calculate(x, y) {
    return x + y
}
...
fn test_function_parameter_in_conditional() {
    ...
    // Check parameters used in conditionals
    let code = r"
fn is_positive(n) {
    if n > 0 {
        return true
...
fn test_function_parameter_in_fstring() {
    ...
    // Check parameters used in f-strings within functions
    let code = r#"
fn greet(name) {
...
fn test_lambda_parameter_usage() {
    ...
    // Check lambda parameters are recognized as used
    let code = r"
let add = |x, y| x + y
let result = add(3, 4)
println(result)
...
fn test_complex_fstring_with_method_calls() {
    ...
    // Check method calls on variables in f-strings
    let code = r#"
let items = [1, 2, 3]
let count = items.len()
...
fn test_actual_unused_variable_detection() {
    ...
    // Check that genuinely unused variables are still detected
    let code = r"
let unused = 42
let used = 10
println(used)
...
fn test_unused_function_parameter_detection() {
    ...
    // Check that genuinely unused parameters are NOT reported as errors
