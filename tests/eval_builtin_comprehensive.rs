...
fn test_sqrt_positive() {
    ruchy_cmd().arg("-e").arg("println(sqrt(16.0))")
        .assert().success().stdout(predicate::str::contains("4"));
}

#[test]
fn test_sqrt_zero() {
    ruchy_cmd().arg("-e").arg("println(sqrt(0.0))")
        .assert().success().stdout(predicate::str::contains("0"));
}

#[test]
fn test_pow_basic() {
    ruchy_cmd().arg("-e").arg("println(pow(2.0, 8.0))")
        .assert().success().stdout(predicate::str::contains("256"));
}

#[test]
fn test_pow_fractional() {
    ruchy_cmd().arg("-e").arg("println(pow(4.0, 0.5))")
        .assert().success().stdout(predicate::str::contains("2"));
}

#[test]
fn test_abs_negative() {
    ruchy_cmd().arg("-e").arg("println(abs(-42.5))")
        .assert().success().stdout(predicate::str::contains("42.5"));
}
...
fn test_min_two_args() {
    ruchy_cmd().arg("-e").arg("println(min(3.0, 7.0))")
...
fn test_min_chained() {
    ...
    ruchy_cmd().arg("-e").arg("println(min(min(5.0, 2.0), 8.0))")
        .assert().success().stdout(predicate::str::contains("2"));
}

#[test]
fn test_max_two_args() {
    ruchy_cmd().arg("-e").arg("println(max(3.0, 7.0))")
        .assert().success().stdout(predicate::str::contains("7"));
}

#[test]
fn test_max_negative_numbers() {
    ruchy_cmd().arg("-e").arg("println(max(-10.0, -5.0))")
        .assert().success().stdout(predicate::str::contains("-5"));
}

#[test]
fn test_floor_positive() {
    ruchy_cmd().arg("-e").arg("println(floor(3.9))")
        .assert().success().stdout(predicate::str::contains("3"));
}

#[test]
fn test_floor_negative() {
    ruchy_cmd().arg("-e").arg("println(floor(-3.1))")
        .assert().success().stdout(predicate::str::contains("-4"));
}

#[test]
fn test_ceil_positive() {
    ruchy_cmd().arg("-e").arg("println(ceil(3.1))")
        .assert().success().stdout(predicate::str::contains("4"));
}

#[test]
fn test_ceil_negative() {
    ruchy_cmd().arg("-e").arg("println(ceil(-3.9))")
        .assert().success().stdout(predicate::str::contains("-3"));
}

#[test]
fn test_round_half_up() {
    ruchy_cmd().arg("-e").arg("println(round(2.5))")
        .assert().success().stdout(predicate::str::contains("3"));
}

#[test]
fn test_round_half_down() {
    ruchy_cmd().arg("-e").arg("println(round(2.4))")
        .assert().success().stdout(predicate::str::contains("2"));
}
...
#[test]
fn test_len_string() {
    ruchy_cmd().arg("-e").arg("println(len(\"hello world\"))")
        .assert().success().stdout(predicate::str::contains("11"));
}

#[test]
fn test_len_array() {
    ruchy_cmd().arg("-e").arg("println(len([1, 2, 3, 4, 5]))")
        .assert().success().stdout(predicate::str::contains("5"));
}

#[test]
fn test_len_empty_string() {
    ruchy_cmd().arg("-e").arg("println(len(\"\"))")
        .assert().success().stdout(predicate::str::contains("0"));
}
...
fn test_range_single_arg() {
    ruchy_cmd().arg("-e").arg("let r = range(5); println(len(r))")
        .assert().success().stdout(predicate::str::contains("5"));
}

#[test]
fn test_range_two_args() {
    ruchy_cmd().arg("-e").arg("let r = range(2, 7); println(len(r))")
        .assert().success().stdout(predicate::str::contains("5"));
}

#[test]
fn test_range_with_step() {
    ruchy_cmd().arg("-e").arg("let r = range(0, 10, 2); println(len(r))")
        .assert().success().stdout(predicate::str::contains("5"));
}

#[test]
fn test_reverse_array() {
    ruchy_cmd().arg("-e").arg("let r = reverse([1, 2, 3]); println(r[0])")
        .assert().success().stdout(predicate::str::contains("3"));
}

#[test]
fn test_assert_eq_pass() {
    ruchy_cmd().arg("-e").arg("assert_eq(42, 42); println(\"OK\")")
        .assert().success().stdout(predicate::str::contains("OK"));
}

#[test]
fn test_assert_eq_fail() {
    ruchy_cmd().arg("-e").arg("assert_eq(42, 99)")
        .assert().failure();
}

#[test]
fn test_assert_true() {
    ruchy_cmd().arg("-e").arg("assert(true); println(\"OK\")")
        .assert().success();
}

#[test]
fn test_assert_false() {
    ruchy_cmd().arg("-e").arg("assert(false)")
        .assert().failure();
}

#[test]
fn test_zip_two_arrays() {
    ruchy_cmd().arg("-e").arg("let z = zip([1, 2], [\"a\", \"b\"]); println(len(z))")
        .assert().success().stdout(predicate::str::contains("2"));
}

#[test]
fn test_enumerate_array() {
    ruchy_cmd().arg("-e").arg("let e = enumerate([\"a\", \"b\", \"c\"]); println(len(e))")
        .assert().success().stdout(predicate::str::contains("3"));
}
...
fn test_print_no_newline() {
    ...
    ruchy_cmd().arg("-e").arg("print(\"A\"); print(\"B\")")
        .assert().success().stdout(predicate::str::contains("\"A\"\"B\""));
}

#[test]
fn test_dbg_returns_value() {
    ruchy_cmd().arg("-e").arg("let x = dbg(123); println(x)")
...
fn integration_assertions() {
    ruchy_cmd().arg("-e")
        ...
        .assert().success().stdout(predicate::str::contains("All assertions passed"));
}
...
fn edge_case_sqrt_large_number() {
    ruchy_cmd().arg("-e").arg("println(sqrt(1000000.0))")
        .assert().success().stdout(predicate::str::contains("1000"));
}

#[test]
fn edge_case_pow_zero_exponent() {
    ruchy_cmd().arg("-e").arg("println(pow(999.0, 0.0))")
...
fn edge_case_min_same_values() {
    ...
    ruchy_cmd().arg("-e").arg("println(min(42.0, 42.0))")
...
fn edge_case_max_same_values() {
    ...
    ruchy_cmd().arg("-e").arg("println(max(42.0, 42.0))")
        .assert().success().stdout(predicate::str::contains("42"));
}

#[test]
fn edge_case_range_zero() {
    ruchy_cmd().arg("-e").arg("println(len(range(0)))")
        .assert().success().stdout(predicate::str::contains("0"));
}

#[test]
fn edge_case_reverse_empty() {
    ruchy_cmd().arg("-e").arg("println(len(reverse([])))")
        .assert().success().stdout(predicate::str::contains("0"));
}
