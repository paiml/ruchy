...
fn test_transpile_if_else() {
    ...
    let test_cases = [
        ...
        ("if x > 0 { x } else { -x }", vec!["if", "x", ">", "0", "x", "else", "-", "x"]),
        ("if a && b { 1 } else { 2 }", vec!["if", "a", "&&", "b", "1", "else", "2"]),
    ];
...
fn test_transpile_blocks() {
    ...
    let test_cases = [
        ("{ 42 }", vec!["42"]),
        ("{ let x = 1; x + 2 }", vec!["let", "x", "=", "1", "x", "+", "2"]),
        ("{ let a = 5; let b = 3; a * b }", vec!["let", "a", "5", "let", "b", "3", "a", "*", "b"]),
    ];
...
fn test_transpile_functions() {
    ...
    let test_cases = [
        ...
        ("fun test() -> bool { true }", vec!["fn", "test", ">", "bool", "true"]),  // Simplified
        ("fun mul(a: f64) -> f64 { a * 2.0 }", vec!["fn", "mul", "a", "f64", ">", "f64", "*", "2.0"]),
    ];
...
fn test_transpile_match() {
    ...
    let result = transpiler.transpile(&ast).expect("Failed to transpile match");
    let transpiled = result.to_string();
...
fn test_transpile_arrays() {
    ...
    let result = transpiler.transpile(&ast).expect("Failed to transpile index");
    let transpiled = result.to_string();
...
fn test_transpile_structs() {
    ...
    let result = transpiler.transpile(&ast).expect("Failed to transpile struct");
    let transpiled = result.to_string();
...
fn test_transpile_method_calls() {
    ...
    for (input, expected_method) in test_cases {
        ...
        assert!(
            ...
            input, expected_method, transpiled
        );
    }
}
...
fn test_transpile_for_loops() {
    ...
    let test_cases = [
        ...
        ("for x in arr { x * 2 }", vec!["for", "x", "in", "arr", "x", "*", "2"]),
    ];
...
fn test_transpile_to_program() {
    ...
    let test_cases = [
        ...
        ("fun main() { println(42) }", vec!["fn main", "println", "42"]),
        
        // Block with multiple statements
        ("{ let x = 1; x + 2 }", vec!["fn main", "let x", "1", "x + 2"]),
    ];
...
fn test_transpile_string_interpolation() {
    ...
    let test_cases = [
        ...
        (r#"f"Result: {1 + 2}""#, vec!["format", "Result", "1", "+", "2"]),
    ];
...
fn test_transpile_async_await() {
    ...
    let test_cases = [
        ...
        ("async fun test() -> i32 { 42 }", vec!["async", "fn", "test", "->", "i32", "42"]),
    ];
...
fn test_transpile_tuples() {
    ...
    let tuple_tests = [
        ("(1, 2)", vec!["(", "1", ",", "2", ")"]),
        ("(true, \"hello\", 42)", vec!["(", "true", ",", "\"hello\"", ",", "42", ")"]),
    ];
    ...
    let result = transpiler.transpile(&ast).expect("Failed to transpile unit tuple");
    let transpiled = result.to_string();
    assert!(
        transpiled.contains("()"),
...
fn test_transpile_field_access() {
    ...
    let test_cases = [
        ...
        ("config.settings.value", vec!["config", ".", "settings", ".", "value"]),
    ];
...
fn test_transpile_type_annotations() {
    ...
    let test_cases = [
        ...
        ("let s: String = \"hello\"", vec!["let", "s", "String", "\"hello\""]),
        ("let opt: Option<i32> = None", vec!["let", "opt", "Option", "<", "i32", ">", "None"]),
    ];
