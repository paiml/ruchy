...
fn test_parse_generic_function() -> Result<()> {
    ...
    match &ast.kind {
        ExprKind::Function { name, type_params, .. } => {
...
fn test_parse_list_comprehension() -> Result<()> {
    ...
    match &ast.kind {
        ExprKind::ListComprehension { element: _, variable: _, iterable: _, condition } => {
...
fn test_parse_bitwise_operations() -> Result<()> {
    ...
    for input in cases {
        ...
        assert!(matches!(ast.kind, ExprKind::Binary { .. }), "Failed for input: {}", input);
    }
