...
use ruchy::wasm::notebook::{NotebookRuntime, ChartConfig, InteractiveConfig};
...
fn test_notebook_commit_system() {
    ...
    let commit1 = runtime.commit_notebook("Initial notebook commit", None).unwrap();
    assert!(!commit1.hash.is_empty(), "Should generate commit hash");
    assert_eq!(commit1.message, "Initial notebook commit");
    assert!(commit1.parent.is_none(), "First commit should have no parent");
    ...
    let commit2 = runtime.commit_notebook("Add calculation", Some(&commit1.hash)).unwrap();
    assert_ne!(commit2.hash, commit1.hash, "Should have different hash");
    assert_eq!(commit2.parent, Some(commit1.hash.clone()), "Should reference parent");
...
fn test_notebook_branching() {
    ...
    let feature_commit = runtime.commit_notebook("Feature implementation", Some(&main_commit.hash)).unwrap();
    ...
    assert_eq!(main_cells_obj.len(), 1, "Main should have original cell only");
...
fn test_notebook_tagging() {
    ...
    let tag = runtime.create_tag("v1.0.0", &commit.hash, "First stable release").unwrap();
    ...
    assert!(cells.contains("Release Version"), "Should restore tagged version");
...
fn test_notebook_diff() {
    ...
    assert_eq!(diff.removed_cells.len(), 0, "No cells removed (both have 3)");
    assert_eq!(diff.modified_cells.len(), 1, "One cell modified at position 2");
    assert!(!diff.has_conflicts, "No conflicts in non-overlapping changes");
...
fn test_notebook_merge() {
    ...
    runtime_branch1.import_collaborative_state(&base_state).unwrap();
    runtime_branch1.add_cell("code", "let analysis1 = shared * 2");
    
    // Branch 2 adds different analysis
    runtime_branch2.import_collaborative_state(&base_state).unwrap();
    ...
    assert_eq!(merge_result.conflicts.len(), 1, "One variable conflict at position 1");
...
fn test_notebook_publishing() {
    ...
    // Publish notebook
    let publish_result = runtime.publish_notebook(
    ...
    assert!(!publish_result.notebook_id.is_empty(), "Should generate notebook ID");
    assert!(!publish_result.share_url.is_empty(), "Should generate share URL");
    assert!(publish_result.published_at > 0, "Should have publication timestamp");
    assert_eq!(publish_result.visibility, "public");
    
    // Update published notebook
    runtime.add_cell("markdown", "## Updated Section");
    let update_result = runtime.update_published_notebook(&publish_result.notebook_id).unwrap();
...
fn test_notebook_discovery() {
    ...
    let search_results = discovery.search_notebooks("data analysis tutorial").unwrap();
    assert!(search_results.results.len() >= 0, "Should return search results");
    
    // Filter by tags
    let filtered = discovery.filter_by_tags(vec!["beginner", "tutorial"]).unwrap();
    assert!(filtered.results.iter().all(|n| 
        n.tags.contains(&"beginner".to_string()) || 
    ...
    if !search_results.results.is_empty() {
        let notebook = discovery.get_notebook(&search_results.results[0].id).unwrap();
...
fn test_notebook_templates() {
    ...
    for template_name in expected_templates {
        assert!(templates.iter().any(|t| t.name == template_name),
            "Should have {} template", template_name);
    }
    ...
    assert!(cells.contains("## Exploratory Analysis"), "Should have analysis section");
...
fn test_custom_template_creation() {
    ...
    runtime.add_cell("code", "// TODO: Import your data here");
    runtime.add_cell("markdown", "## Step 1: Data Preparation");
    runtime.add_cell("code", "// TODO: Clean and prepare data");
    
    // Save as template
    let template = runtime.save_as_template(
        "custom_analysis",
    ...
    let cells = runtime.get_cells();
    assert!(cells.contains("Custom Analysis Template"), "Should load custom template");
...
fn test_notebook_search_indexing() {
    ...
    assert!(results[0].relevance_score > 0.5, "Should have good relevance");
...
fn test_semantic_search() {
    ...
    assert!(!results.is_empty(), "Should find semantically related content");
    
    // Should match visualization even when searching for "plotting"
    let related = runtime.semantic_search("plotting data").unwrap();
    assert!(related.iter().any(|r| r.content.contains("Visualization")),
...
fn test_data_visualization_charts() {
    ...
    let data_cell = runtime.add_cell("code", "let chart_data = [[1, 10], [2, 20], [3, 15], [4, 25]]");
    runtime.execute_cell(&data_cell).unwrap();
    
    // Create line chart
    let line_chart = runtime.create_chart("line", "chart_data", ChartConfig {
    ...
    // Create bar chart
    let bar_chart = runtime.create_chart("bar", "chart_data", ChartConfig {
    ...
    // Create pie chart
    let pie_data_cell = runtime.add_cell("code", "let categories = [['A', 30], ['B', 45], ['C', 25]]");
    runtime.execute_cell(&pie_data_cell).unwrap();
    
    let pie_chart = runtime.create_chart("pie", "categories", ChartConfig {
...
fn test_interactive_visualizations() {
    ...
    let scatter = runtime.create_interactive_viz("scatter", "dataset", InteractiveConfig {
        enable_zoom: true,
        enable_pan: true,
        enable_hover: true,
...
fn test_notebook_plugin_system() {
    ...
    for plugin_name in &expected_plugins {
        assert!(plugins.iter().any(|p| p.name == *plugin_name),
            "Should have {} plugin", plugin_name);
    }
    ...
    // Verify plugin effect (formatting should clean up code)
    let cells = runtime.get_cells();
...
fn test_custom_plugin_development() {
    ...
    let custom_plugin = runtime.register_plugin(
        "custom_analyzer",
    ...
    // Plugin hook system
    runtime.add_plugin_hook("custom_analyzer", "pre_execute", |cell_content| {
...
fn test_collaborative_workflow() {
    ...
    let alice_commit = alice.commit_notebook("Initial project setup", None).unwrap();
    alice.publish_notebook("Team Project", "Collaborative analysis", 
        vec!["team", "project"], "MIT", false).unwrap();
    ...
    bob.commit_notebook("Add subset analysis", Some(&alice_commit.hash)).unwrap();
    
    // Alice continues on main
    alice.add_cell("code", "let alice_visualization = create_charts(project_data)");
    alice.commit_notebook("Add visualizations", Some(&alice_commit.hash)).unwrap();
    
    // Bob creates pull request (simulated)
    let pr = bob.create_pull_request("bob-analysis", "main", 
        "Add subset analysis feature").unwrap();
...
fn test_end_to_end_notebook_lifecycle() {
    ...
    runtime.create_chart("line", "viz_data", ChartConfig {
    ...
    runtime.create_tag("v1.0", &commit.hash, "First complete version").unwrap();
    ...
    // 7. Publish
    let published = runtime.publish_notebook(
