...
fn test_wasm_module_size_under_200kb() {
    ...
    if wasm_path.exists() {
        ...
        assert!(size_kb < 200, "WASM module size {}KB exceeds 200KB limit", size_kb);
    }
}
...
fn test_browser_api_bindings_exist() {
    ...
    assert!(content.contains("#[wasm_bindgen]"), "Missing wasm_bindgen attributes");
    assert!(content.contains("wasm_bindgen(constructor)"), "Missing constructor binding");
