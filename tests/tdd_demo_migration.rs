...
fn test_demo_parser_exists() {
    // Test that we have a demo parser module available
    ...
    assert!(demo_files.len() >= 28, "Expected at least 28 demo files, found {}", demo_files.len());
}

#[test] 
fn test_simple_demo_to_notebook_conversion() {
    // Test converting a simple demo file to notebook format
...
let x = 10"#;
    ...
    assert_eq!(notebook.cells.len(), 2, "Expected 2 cells (filtering REPL commands)");
...
fn test_notebook_execution_after_conversion() {
    ...
    // Execute first cell
    let result1 = runtime.execute_cell_with_session("cell1", &notebook.cells[0].source).unwrap();
    ...
    let result2 = runtime.execute_cell_with_session("cell2", &notebook.cells[1].source).unwrap(); 
    assert_eq!(result2.value, "50");
}

#[test]
fn test_all_demo_files_convertible() {
    // Test that all 34 demo files can be converted without errors
    let demo_files = find_demo_files();
    ...
    assert!(conversion_count >= 28, "Expected to convert at least 28 demos, got {}", conversion_count);
}

#[test]
fn test_notebook_format_compliance() {
    // Test that converted notebooks match expected JSON structure
    let demo_content = "42\nlet x = 10";
...
fn test_notebook_metadata_generation() {
    ...
    assert_eq!(notebook.metadata.get("language_info")
        .and_then(|v| v.as_object())
        .and_then(|o| o.get("name"))
        .and_then(|n| n.as_str()), Some("ruchy"));
