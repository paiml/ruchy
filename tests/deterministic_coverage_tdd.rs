...
use ruchy::runtime::deterministic::{DeterministicRng, MockTime};
use ruchy::runtime::repl::Repl;
use ruchy::runtime::replay::{DeterministicRepl, Divergence, ResourceUsage, StateCheckpoint};
...
fn test_restore_with_values() {
    ...
    checkpoint
        .bindings
        .insert("test_var".to_string(), "42".to_string());
...
fn test_validation_divergence_detection() {
    ...
    let has_state_divergence = validation
        .divergences
        .iter()
        .any(|d| matches!(d, Divergence::State { .. }));
...
fn test_restore_value_types() {
    ...
    // Check restoring different value types
    let mut checkpoint = StateCheckpoint {
        bindings: HashMap::new(),
        type_environment: HashMap::new(),
        state_hash: "test".to_string(),
    ...
    checkpoint
        .bindings
        .insert("integer".to_string(), "123".to_string());
    checkpoint
        .bindings
        .insert("string".to_string(), "hello".to_string());
