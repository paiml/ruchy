...
use ruchy::frontend::ast::{DataFrameColumn, DataFrameOp, JoinType, AggregateOp, Expr, ExprKind, Literal, Span};
...
fn test_dataframe_literal_basic() {
    ...
    let df_expr = create_expr(ExprKind::DataFrame { columns: df_columns });
...
fn test_dataframe_select_operation() {
    ...
    let df_expr = create_expr(ExprKind::DataFrame {
        columns: vec![
            ...
            DataFrameColumn {
                name: "name".to_string(),
                values: vec![create_expr(ExprKind::Literal(Literal::String("test".to_string())))],
            },
        ],
    });
...
fn test_mixed_column_types() {
    ...
    let mixed_df = create_expr(ExprKind::DataFrame { columns: df_columns });
