...
fn test_checkpoint_restore() {
    ...
    DeterministicRepl::restore(&mut repl_mut, &checkpoint).unwrap();
...
mod property_tests {
    ...
    proptest! {
        ...
        fn test_checkpoint_restore_idempotency(operations in prop::collection::vec("let [a-z] = [0-9]", 0..10)) {
            ...
            let checkpoint2 = DeterministicRepl::checkpoint(&repl);

            // Checkpoints should be consistent (content may vary, but structure should be valid)
            prop_assert!(!checkpoint.state_hash.is_empty(), "Checkpoint should not be empty");
            prop_assert!(!checkpoint2.state_hash.is_empty(), "Restored checkpoint should not be empty");
        }
    }
}
