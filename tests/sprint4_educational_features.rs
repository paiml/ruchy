...
impl EducationalPlatform {
    ...
    fn submit_assignment(&mut self, _student_id: &str, submission: StudentSubmission) -> Result<(), String> {
    ...
    fn auto_grade(&self, submission: &StudentSubmission) -> Grade {
        ...
        for test_case in &assignment.test_cases {
            if submission.notebook.cells.iter().any(|c| c.id == test_case.cell_id) {
...
impl Grader {
    ...
    fn grade_with_rubric(&self, _submission: &StudentSubmission, rubric: &[RubricItem], scores: &[(String, u32)]) -> Grade {
        let mut total = 0;
...
impl InteractiveTutorial {
    ...
    fn validate_step(&self, step_id: &str, submission: &str) -> StepResult {
        ...
        let is_correct = match &step.validation {
            ...
            ValidationRule::TestCase { input: _, expected: _ } => submission.contains("fn double"),
...
impl AdaptiveHintSystem {
    ...
    fn record_attempt(&mut self, student: &str, problem: &str, attempt: &str, success: bool) {
        self.attempts.push((student.to_string(), problem.to_string(), attempt.to_string(), success));
    }
    
    fn get_hint(&self, student: &str, problem: &str) -> String {
        let student_attempts = self.attempts.iter()
