...
fn test_score_spec_compliance_complexity_boundaries() {
    // Check that our implementation matches the spec from docs/specifications/ruchy-scoring-spec.md
...
fn test_score_spec_grade_boundaries() {
    // Check grade boundaries from spec (lines 684-696)
    let test_cases = vec![
        (0.98, "A+"), // [0.97, 1.00]
        (0.95, "A"),  // [0.93, 0.97)
        (0.91, "A-"), // [0.90, 0.93)
...
fn test_score_parameter_penalty() {
    // Check that excessive parameters are heavily penalized
    let dir = TempDir::new().unwrap();
    let file_path = dir.path().join("test.ruchy");
...
fn test_score_nesting_depth_penalty() {
    // Check that deep nesting is heavily penalized
    let dir = TempDir::new().unwrap();
    let file_path = dir.path().join("test.ruchy");
