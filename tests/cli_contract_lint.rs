...
fn cli_lint_clean_code_exits_zero() {
    ...
    ruchy_cmd().arg("lint").arg(&file).assert().success(); // Exit code 0
}
...
fn cli_lint_syntax_error_exits_nonzero() {
    ...
    ruchy_cmd().arg("lint").arg(&file).assert().failure(); // Exit code != 0
}
...
fn cli_lint_unused_variable_warning() {
    ...
    let output = ruchy_cmd().arg("lint").arg(&file).assert().success(); // Warnings don't cause failure
...
fn cli_lint_shadowing_detection() {
    ...
    ruchy_cmd().arg("lint").arg(&file).assert().success(); // Shadowing is warning, not error
}
...
fn cli_lint_empty_file_fails() {
    ...
    ruchy_cmd().arg("lint").arg(&file).assert().failure(); // Empty file is error
}
...
fn cli_lint_comment_only_fails() {
    ...
    ruchy_cmd().arg("lint").arg(&file).assert().failure(); // Comment-only is error
}
