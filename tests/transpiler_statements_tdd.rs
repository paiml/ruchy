...
fn test_transpile_impl_block() {
    let transpiler = Transpiler::new();
    let mut parser = Parser::new("impl Point { fun new(x: f64, y: f64) -> Point { Point { x, y } } }");
    let ast = parser.parse().unwrap();
...
fn test_transpile_trait_impl() {
    let transpiler = Transpiler::new();
    let mut parser = Parser::new("impl Display for Point { fun fmt(&self, f: &mut Formatter) -> Result { } }");
    let ast = parser.parse().unwrap();
