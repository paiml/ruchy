...
fn test_end_to_end_pattern_matching() {
    ...
    assert!(rust_str.contains("Some"));
    assert!(rust_str.contains("None"));
}
...
fn test_end_to_end_list_comprehension() {
    ...
    let rust_str = rust_code.to_string();
    assert!(rust_str.contains("filter") || rust_str.contains("map") || rust_str.contains("collect"));
}
...
fn test_end_to_end_import_statement() {
    ...
    // Imports should be converted to use statements
    let rust_str = rust_code.to_string();
    assert!(rust_str.contains("use"));
}
...
fn test_compile_generated_rust_code() {
    ...
    fs::write(&lib_path, rust_code.to_string()).expect("Failed to write Rust code");
    ...
    assert!(output.status.success(), "Generated Rust code failed to compile: {}", 
...
fn test_large_file_handling() {
    ...
    for i in 0..1000 {
        large_input.push_str(&format!("let var_{} = {};\n", i, i));
    }
