...
fn test_basic_state_persistence() {
    ...
    let result1 = runtime.execute_cell_with_session("cell1", "let x = 42").unwrap();
    ...
    let result3 = runtime.execute_cell_with_session("cell3", "let y = x * 2").unwrap();
...
// Test 2: Function definitions persist across cells
#[test]
fn test_function_persistence() {
    ...
    runtime.execute_cell_with_session("cell3", "fun double_add(a, b) { add(a, b) * 2 }").unwrap();
    
    // Cell 4: Use the new function
    let result = runtime.execute_cell_with_session("cell4", "double_add(5, 6)").unwrap();
    assert!(result.success);
    assert_eq!(result.value, "22");
}

// Test 3: Semantic dependency tracking
#[test]
fn test_semantic_dependency_tracking() {
    let mut runtime = NotebookRuntime::new().unwrap();
    
    // Define x
    runtime.execute_cell_with_session("cell1", "let x = 10").unwrap();
    
    // Define y depending on x
    runtime.execute_cell_with_session("cell2", "let y = x * 2").unwrap();
    ...
    // Redefine x
    runtime.execute_cell_with_session("cell3", "let x = 20").unwrap();
...
// Test 4: Reactive execution cascade
#[test]
fn test_reactive_cascade_execution() {
    ...
    runtime.execute_cell_with_session("c1", "let x = 10").unwrap();
    runtime.execute_cell_with_session("c2", "let y = x * 2").unwrap();
    runtime.execute_cell_with_session("c3", "let z = y + 5").unwrap();
...
fn test_transactional_rollback() {
    ...
    runtime.execute_cell_with_session("cell2", "let y = 10").unwrap();
...
// Test 7: Hierarchical namespace modules
#[test]
...
// Test 8: Execution plan preview for reactive mode
#[test]
fn test_execution_plan_preview() {
    let mut runtime = NotebookRuntime::new().unwrap();
    
    // Set up dependency chain
    runtime.execute_cell_with_session("c1", "let a = 1").unwrap();
    runtime.execute_cell_with_session("c2", "let b = a * 2").unwrap();
    runtime.execute_cell_with_session("c3", "let c = b + a").unwrap();
    runtime.execute_cell_with_session("c4", "let d = c * 2").unwrap();
...
// Test 9: State inspection API
#[test]
fn test_state_inspection_api() {
    let mut runtime = NotebookRuntime::new().unwrap();
    
    // Execute some cells
    runtime.execute_cell_with_session("c1", "let x = 42").unwrap();
    runtime.execute_cell_with_session("c2", "let y = x * 2").unwrap();
    runtime.execute_cell_with_session("c3", "fun add(a, b) { a + b }").unwrap();
...
fn test_memory_usage_tracking() {
    ...
    runtime.execute_cell_with_session("c1", "let data = [1, 2, 3, 4, 5]").unwrap();
    runtime.execute_cell_with_session("c2", "let matrix = [[1, 2], [3, 4]]").unwrap();
