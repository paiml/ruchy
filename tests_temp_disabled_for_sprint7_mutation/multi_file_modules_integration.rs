...
fn test_basic_multi_file_program() -> Result<()> {
    ...
    // Check the generated Rust code contains expected elements
    assert!(
        rust_string.contains("mod math"),
        "Should contain math module declaration"
    );
    assert!(
        rust_string.contains("pub fn add"),
        "Should contain add function"
    );
    assert!(
        rust_string.contains("pub fn multiply"),
        "Should contain multiply function"
    );
    assert!(
        rust_string.contains("fn main"),
        "Should contain main function"
    );
    assert!(
        rust_string.contains("math :: add"),
        "Should contain qualified function call"
    );
...
fn test_module_not_found_fallback() -> Result<()> {
    ...
    assert!(
        error_msg.contains("nonexistent_module"),
