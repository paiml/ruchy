...
fn test_repl_eval_recording_scenarios() {
    ...
    // Check evaluating various inputs
    let test_cases = vec![
        ("5 + 3", true),
        ("let x = 10", true),
...
fn test_edge_cases() {
    ...
    recorder.record_output(Ok(ruchy::runtime::Value::String(std::rc::Rc::from(
        "result",
    ))));
