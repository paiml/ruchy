...
"#;
    ...
    assert!(
        ...
        "Should successfully score excellent code"
    );
...
"#;
    ...
    assert!(
        output.status.success(),
        "Should successfully score terrible code"
    );

    let stdout = String::from_utf8_lossy(&output.stdout);
    let score = extract_score_from_output(&stdout);

    // This will initially fail because terrible code currently gets ~0.84
...
";
    ...
    let terrible_score =
        extract_score_from_output(&String::from_utf8_lossy(&terrible_output.stdout));
