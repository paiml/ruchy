...
fn regression_while_loop_off_by_one() {
    ...
    // Check 1: Exact original failing case (now using var for mutable bindings)
    let result = repl
        .eval("var i = 0; var count = 0; while i < 3 { count = count + 1; i = i + 1 }; count")
        .unwrap();
    assert_eq!(result, "3", "While loop should execute exactly 3 times");

    // Check 2: While loop should return Unit, not body value
    let result = repl
        .eval("var i = 0; while i < 1 { i = i + 1; 42 }")
        .unwrap();
    assert_eq!(result, "()", "While loop should return Unit, not 42");

    // Check 3: Multiple iterations with different body values
    let result = repl
        .eval("var i = 0; while i < 2 { i = i + 1; i * 10 }")
        .unwrap();
    assert_eq!(
        result, "()",
        "While loop should return Unit, not last body value"
    );
}
...
fn regression_object_items_transpilation() {
    ...
    // Check via file execution (this used to fail)
...
fn regression_test_compilation_errors() {
    ...
    assert!(
        result.is_ok(),
        "Raw string test code should compile: {result:?}"
    );
}
...
fn regression_make_lint_underscore_binding() {
    ...
    // Check lint passes now with timeout
...
fn regression_coverage_script_flags() {
    ...
    // Check that coverage commands work individually with timeout
    ...
    assert!(
        !stderr.contains("may not be used together"),
...
fn regression_template_future_bug() {
    ...
    // Check that demonstrates the bug is fixed
...
fn regression_performance_not_degraded() {
    ...
    assert!(
        ...
        "Simple arithmetic took too long: {}ms",
        duration.as_millis()
    );
    ...
    assert!(
        ...
        "Function call took too long: {}ms",
        duration.as_millis()
    );
}
