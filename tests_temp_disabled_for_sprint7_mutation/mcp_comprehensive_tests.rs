...
fn test_ruchy_mcp_basic_functionality() {
    ...
    assert_ne!(
        std::ptr::addr_of!(mcp) as usize,
        std::ptr::addr_of!(mcp2) as usize
    );
}
...
fn test_type_validation_invalid() -> anyhow::Result<()> {
    ...
    // Test invalid values
    assert!(mcp
        .validate_against_type(&json!("not_a_number"), "count")
        .is_err());
...
fn test_list_type_validation() -> anyhow::Result<()> {
    ...
    // Test valid list
    assert!(mcp
        .validate_against_type(&json!([1, 2, 3]), "numbers")
        .is_ok());

    // Test invalid list
    assert!(mcp
        .validate_against_type(&json!([1, "two", 3]), "numbers")
        .is_err());
...
fn test_float_type_validation() -> anyhow::Result<()> {
    ...
    assert!(mcp
        .validate_against_type(&json!(42.5), "temperature")
        .is_ok());
    assert!(mcp
        .validate_against_type(&json!(0.0), "temperature")
        .is_ok());
    assert!(mcp
        .validate_against_type(&json!(-273.15), "temperature")
        .is_ok());

    // Test invalid float values
    assert!(mcp
        .validate_against_type(&json!("not_a_float"), "temperature")
        .is_err());
    assert!(mcp
        .validate_against_type(&json!(true), "temperature")
        .is_err());
...
fn test_ruchy_mcp_tool_creation() {
    let handler = |_value: Value| -> anyhow::Result<Value> { Ok(json!("test_result")) };
...
fn test_ruchy_mcp_tool_with_input_type() {
    let handler = |_value: Value| -> anyhow::Result<Value> { Ok(json!("result")) };
...
fn test_ruchy_mcp_tool_with_output_type() {
    let handler = |_value: Value| -> anyhow::Result<Value> { Ok(json!(42)) };
...
fn test_ruchy_mcp_tool_fully_typed() {
    let handler = |_value: Value| -> anyhow::Result<Value> { Ok(json!("processed")) };
...
fn test_multiple_type_registrations() -> anyhow::Result<()> {
    ...
    mcp.register_type(
        "scores".to_string(),
        MonoType::List(Box::new(MonoType::Float)),
    );

    // Validate against all registered types
    assert!(mcp.validate_against_type(&json!(123), "user_id").is_ok());
    assert!(mcp
        .validate_against_type(&json!("alice"), "username")
        .is_ok());
    assert!(mcp.validate_against_type(&json!(true), "is_active").is_ok());
    assert!(mcp
        .validate_against_type(&json!(["admin", "user"]), "tags")
        .is_ok());
    assert!(mcp
        .validate_against_type(&json!([95.5, 87.2, 92.0]), "scores")
        .is_ok());
...
fn test_nested_list_validation() -> anyhow::Result<()> {
    ...
    let nested_list_type = MonoType::List(Box::new(MonoType::List(Box::new(MonoType::Int))));
    ...
    assert!(mcp
        .validate_against_type(&invalid_matrix, "matrix")
        .is_err());
...
fn test_mcp_tool_handler() -> anyhow::Result<()> {
    ...
    let tool = RuchyMCPTool::new("echo_tool".to_string(), "Echoes input".to_string(), handler);
...
fn test_numeric_edge_cases() -> anyhow::Result<()> {
    ...
    // Test integer bounds
    assert!(mcp
        .validate_against_type(&json!(i64::MAX), "integer")
        .is_ok());
    assert!(mcp
        .validate_against_type(&json!(i64::MIN), "integer")
        .is_ok());
