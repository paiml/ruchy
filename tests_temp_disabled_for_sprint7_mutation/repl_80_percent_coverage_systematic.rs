...
mod repl_value_fmt_coverage {
    ...
    fn test_value_fmt_comprehensive_all_variants() {
        ...
        let test_values = vec![
            ...
            Value::String(Rc::from("")),                 // Empty string
            Value::String(Rc::from("hello\nworld\ttab")), // Escape sequences
            Value::String(Rc::from("🦀 unicode ∑∞")),     // Unicode
            Value::Bool(true),
            Value::Bool(false),
            // Note: Value::Char not implemented in current Value enum
            Value::Nil,
        ];
        ...
        for value in test_values {
            ...
            assert!(
                !debug_str.is_empty(),
                "Debug should not be empty for: {value:?}"
            );
    ...
    fn test_value_fmt_collections_comprehensive() {
        ...
        let tuples = vec![
            ...
            Value::Tuple(vec![Value::Integer(1), Value::String(Rc::from("test"))]), // Mixed types
        ...
        let hashmaps = vec![
            Value::HashMap(HashMap::new()), // Empty hashmap
            {
                ...
                map.insert(Value::String(Rc::from("key")), Value::Integer(42));
                Value::HashMap(map)
            },
        ];
        ...
        let hashsets = vec![
            Value::HashSet(HashSet::new()), // Empty hashset
            {
                ...
                set.insert(Value::String(Rc::from("item")));
                Value::HashSet(set)
            },
        ];
    ...
    fn test_value_fmt_special_types_comprehensive() {
        ...
        let body_expr = Expr::new(ExprKind::Literal(Literal::Integer(42)), Span::new(0, 2));
