...
fn test_variable_binding_persistence() {
    ...
    // Check variables retain their values
    ...
    // Check arithmetic works
    let sum = repl.eval("x + y").expect("Failed to add x + y");
    assert_eq!(sum, "30");

    // Check println with expressions works
    let result = repl
...
fn test_match_expression_evaluation() {
    // Check that match expressions work correctly
    let mut repl = Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
...
fn test_string_interpolation_complex() {
    // Check f-string interpolation with expressions
    let mut repl = Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
...
fn test_nested_control_flow() {
    // Check complex nested structures work correctly
    let mut repl = Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
...
fn test_list_operations() {
    // Check list creation and iteration
    let mut repl = Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
...
fn test_character_literals() {
    // Check character literal support
    let mut repl = Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
...
fn test_comparison_operators() {
    // Check all comparison operators work
    let mut repl = Repl::new(std::env::temp_dir()).expect("Failed to create REPL");
