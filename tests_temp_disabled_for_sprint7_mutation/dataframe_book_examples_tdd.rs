...
mod dataframe_book_examples_tdd {
    ...
    fn test_dataframe_new_constructor() {
        ...
        assert!(
            result.is_ok(),
            "DataFrame::new() should work, got error: {:?}",
            result
        );

        // Check it returns a DataFrame value
        let output = result.unwrap();
        assert!(
            output.contains("DataFrame") || output.contains("column"),
            "Should return DataFrame, got: {}",
            output
        );
    }
    ...
    fn test_dataframe_columns_method() {
        ...
        assert!(
            result.is_ok(),
            "df.columns() should work, got error: {:?}",
            result
        );
        ...
        assert!(
            output.contains("2"),
            "df.columns() should return 2, got: {}",
            output
        );
    }
    ...
    fn test_dataframe_display() {
        ...
        assert!(
            result.is_ok(),
            "DataFrame creation should work, got error: {:?}",
            result
        );
        ...
        assert!(
            output.contains("product") || output.contains("DataFrame") || output.contains("Widget"),
            "DataFrame should display nicely, got: {}",
            output
        );
    }
}
