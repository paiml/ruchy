...
fn test_str_parameter_transpilation() {
    // Check that Ruchy 'str' type maps to Rust '&str' in parameters
...
fn test_string_literal_no_unnecessary_to_string() {
    // Check that string literals don't get .to_string() when passed to &str parameters
...
fn test_println_format_correctness() {
    // Check that println!("{}", name) generates correct format, not "{} {:?}"
...
fn test_no_unnecessary_hashmap_import() {
    // Check that simple functions don't generate unused HashMap import
...
fn test_no_extra_braces() {
    // Check that function bodies don't get double-wrapped in braces
