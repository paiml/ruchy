...
fn run_differential_test(test: &DifferentialTest) -> Result<(), String> {
    ...
    let repl_output = Command::new("bash")
        .arg("-c")
        .arg(format!(
            "echo '{}' | timeout 10 ./target/release/ruchy repl",
            test.code
        ))
        .output()
    ...
    fs::write(&test_file, test.code).map_err(|e| format!("Failed to write test file: {e}"))?;
...
fn test_error_consistency() {
    ...
    for code in error_cases {
        let repl_result = Command::new("bash")
            .arg("-c")
            .arg(format!(
                "echo '{code}' | timeout 5 ./target/release/ruchy repl 2>&1"
            ))
            .output()
            .unwrap();
        ...
        assert!(
            repl_output.contains("Error:") || repl_output.contains("error:"),
            "REPL should show error for: {code}"
        );
        assert!(
            file_output.contains("Error:") || file_output.contains("error:"),
