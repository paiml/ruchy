...
use ruchy::backend::optimizer::{OptLevel, OptimizationPass, Optimizer};
use ruchy::frontend::ast::{Expr, Program, Stmt};
...
fn test_loop_unrolling() {
    ...
    let for_loop = Stmt::for_loop(
        "i",
        Expr::range(0, 4),
        vec![Stmt::expr(Expr::call("print", vec![Expr::ident("i")]))],
    );
...
fn test_inline_small_function() {
    ...
    optimizer.register_function(
        "small_func",
        1,
        vec![Stmt::return_value(Some(Expr::binary(
            "*",
            Expr::ident("x"),
            Expr::literal(2),
        )))],
...
fn test_tail_call_optimization() {
    ...
    let func = Function {
        ...
        body: vec![Stmt::if_stmt(
            ...
            Some(vec![Stmt::return_value(Some(Expr::binary(
                "*",
                Expr::ident("n"),
                Expr::call(
                    "factorial",
                    vec![Expr::binary("-", Expr::ident("n"), Expr::literal(1))],
                ),
            )))]),
        )],
    };
...
fn test_optimization_pass_ordering() {
    ...
    let cf_idx = passes
        .iter()
        .position(|p| matches!(p, OptimizationPass::ConstantFolding));
    let dce_idx = passes
        .iter()
        .position(|p| matches!(p, OptimizationPass::DeadCodeElimination));
...
impl Optimizer {
    ...
    fn with_level(level: OptLevel) -> Self {
        Self { level }
    }
    ...
    fn optimize_statements(&mut self, stmts: Vec<Stmt>) -> Vec<Stmt> {
        stmts
    }
    fn optimize_stmt(&mut self, stmt: Stmt) -> Stmt {
        stmt
    }
    fn optimize_loop(&mut self, body: Vec<Stmt>) -> Vec<Stmt> {
        body
    }
    ...
    fn has_temp_binding(&self) -> bool {
        false
    }
    fn cse_count(&self) -> usize {
        0
    }
    fn hoisted_count(&self) -> usize {
        0
    }
    fn unrolled_loops(&self) -> usize {
        0
    }
    fn tail_calls_optimized(&self) -> usize {
        0
    }
