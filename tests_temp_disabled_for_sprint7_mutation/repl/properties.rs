...
fn arb_valid_expression() -> impl Strategy<Value = String> {
    prop_oneof![
        ...
        ("[a-z][a-z0-9]*", "[a-z][a-z0-9]*", any::<i32>()).prop_map(
            |(fname, param, body)| format!("fun {fname}({param}: i32) -> i32 {{ {body} }}")
        ),
        // Lists
        prop::collection::vec(any::<i32>(), 0..5).prop_map(|nums| format!(
            "[{}]",
            nums.iter()
                .map(|n| n.to_string())
                .collect::<Vec<_>>()
...
impl ReplTestExt for Repl {
    ...
    fn apply_action(&mut self, action: &ReplAction) -> Result<String, String> {
        match action {
            ReplAction::Eval(expr) => self.eval(expr).map_err(|e| e.to_string()),
            ReplAction::ShowHistory => Ok(self.show_history()),
            ReplAction::ShowType(var) => self.show_type(var).map_err(|e| e.to_string()),
            ReplAction::ShowRust(expr) => self.show_rust(expr).map_err(|e| e.to_string()),
            ReplAction::ShowAst(expr) => self.show_ast(expr).map_err(|e| e.to_string()),
            ReplAction::Clear => {
                self.clear_session();
