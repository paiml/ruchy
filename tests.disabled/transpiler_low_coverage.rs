...
fn test_transpile_result_types() {
    ...
    let test_cases = [
        ...
        (
            ...
            vec!["let", "x", "Result", "i32", "String", "Ok", "42"],
        ),
        // Pattern matching on Result
        (
            r#"match result { Ok(val) => val, Err(e) => 0 }"#,
            vec!["match", "result", "Ok", "val", "Err", "e", "0"],
        ),
    ];
...
fn test_transpile_option_types() {
    ...
    let test_cases = [
        ...
        (
            ...
            vec!["let", "x", "Option", "i32", "Some", "42"],
        ),
        // Pattern matching on Option
        (
            r#"match opt { Some(val) => val, None => 0 }"#,
            vec!["match", "opt", "Some", "val", "None", "0"],
        ),
    ];
...
fn test_transpile_type_inference_edge_cases() {
    ...
    let test_cases = [
        ...
        (
            ...
            vec!["fn", "identity", "<", "T", ">", "x", "T"],
        ),
        // Type alias
        (
            "type Point = (i32, i32)",
            vec!["type", "Point", "i32", "i32"],
        ),
        // Complex generic type
        (
            "let map: HashMap<String, Vec<i32>> = HashMap::new()",
            vec!["let", "map", "HashMap", "String", "Vec", "i32"],
        ),
    ];
...
fn test_transpile_dataframe_operations() {
    ...
    let test_cases = [
        ...
        (
            r#"df![{"col1": [1, 2, 3]}]"#,
            vec!["DataFrame", "col1", "1", "2", "3"],
        ),
        // DataFrame select
        (
            r#"df.select(["col1", "col2"])"#,
            vec!["df", "select", "col1", "col2"],
        ),
        // DataFrame filter
        (
            r#"df.filter(col("age") > 18)"#,
            vec!["df", "filter", "col", "age", ">", "18"],
        ),
    ];
...
fn test_transpile_actors() {
    ...
    let test_cases = [
        // Actor definition
        (
            r#"actor Counter { state count: i32 = 0 }"#,
            vec!["struct", "Counter", "count", "i32"],
        ),
        // Actor message send
...
fn test_transpile_complex_nested() {
    ...
    let test_cases = [
        ...
        // Nested match
        (
            r#"match x { Some(y) => match y { 0 => "zero", _ => "other" }, None => "none" }"#,
            vec![
                "match", "x", "Some", "y", "match", "y", "0", "zero", "other", "None", "none",
            ],
        ),
...
fn test_transpile_edge_cases() {
    ...
    let test_cases = [
        ...
        (
            "let very_long_identifier_name_that_exceeds_normal_length = 42",
            vec![
                "let",
                "very_long_identifier_name_that_exceeds_normal_length",
                "42",
            ],
        ),
        // Unicode identifiers
