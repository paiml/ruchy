...
- **(behaviour)** A Ruchy name that is a Rust reserved word (`box`, `do`, `typeof`,
  `extern`, ...) is emitted as a raw identifier (`r#box`) when it names a mutable global,
  a function, method or closure parameter, a `for` variable or a pattern binding. Before,
  `ruchy transpile` and `ruchy compile` failed on such programs (RAWIDENT-2).
- **(behaviour)** A function parameter, closure parameter, `for` variable, pattern or `let`
  with the same name as a mutable global now reads and writes the local. Before, the body
...
- **Binary size budget gate** ([EMBED-009]): `tests/sovereign_binary_size_budget.rs`
  validates Success Criterion #6 — release binary stays under 14.19 MB
  (12.4 MB 4.x baseline + 20%). Currently 8.45 MB (40% headroom).
...
- **[QUALITY-001] PMAT v2.200.0 Full Integration** Complete integration with paiml-mcp-agent-toolkit
  ...
  - **TDG Score**: 94.9/100 (A grade) for src/ directory
  - **Security Audit**: cargo audit run - 1 unmaintained warning (paste via polars, low risk)
...
- **[QUALITY-001] Code Quality Improvements** cargo fmt and clippy fixes
  - **Formatting**: Ran cargo fmt --all (5 test files reformatted)
